...
extern int nexus_keyboard_interrupt(void *unused);
...
void handle_scancode(unsigned char scancode, int down)
{
	...
	switch (scancode) {
	case 0x01 /* ESC */ : Debug_KCommand(1, (char *[]) { "reboot", NULL }); break;
	case 0x3b /* F1  */ : Debug_KCommand(1, (char *[]) { "meminfo", NULL }); break;
	case 0x3c /* F2  */ : break;
	case 0x3d /* F3  */ : Debug_KCommand(1, (char *[]) { "top", NULL }); break;
	case 0x3e /* F4  */ : Debug_KCommand(1, (char *[]) { "log", NULL }); break;
	}
#endif
	...
	if (!rep ||
		...
		int shift_final = (0 /* shift_state : hacky bugfix */ | kbd->slockstate) ^ kbd->lockstate;
...
void put_queue(int ch)
{
  ...
  sendchar = galloc(1); // BUG: can block. XXX move keyboard to userspace
  sendchar[0] = shift_char(ch);

  if (IPC_Send(default_keyboard_port, sendchar, 1))
    printk("[keyboard] send failed\n");
}
...
int keyboard_init(void)
{
	...
	kbd_init_hw();	// resolves to pckbd_init_hw
	
#ifdef __NEXUSKERNEL__
	nxirq_get(1, nexus_keyboard_interrupt, NULL);
#endif
	return 0;
}
...
void kbd_drv_context_change_mode(struct kbd_drv_context *ctx, 
	...
	if (focused) {
		...
		kbd->modeflags = ctx->modeflags;
		kbd->kbdmode = ctx->kbdmode;
	}
}
