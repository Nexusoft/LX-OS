...
void alt_tab_event(int shift_on) {
  Console_Switch(shift_on ? 1 /* left */ : 0 /* right */);
}
...
handle_kbd_event(void)
{
	...
	while ((--work > 0) && (status & KBD_STAT_OBF)) {
		...
		/* Error bytes must be ignored to make some hardware work */
#if 0
		/* Ignore error bytes */
...
int nexus_keyboard_interrupt(void *unused) 
{
...
int __init psaux_init(void)
{
	...
	nd_set_protocol(MPROT_DEFAULT);

	printf("[kbd] Mouse initialized\n");
	return 0;
}
