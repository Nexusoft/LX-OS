...
/* NEXUS driver taken from Linux 2.4.22. 
   NONEXUS definitions disables parts

   Please do not edit code willy-nilly. mark updates to the original clearly

   The exception to this rule (so far) is that all used instances of
   spin_unlock_irqrestore have been changed into spin_unlock

   XXX reenable mixer_ioctl (see probe)
 */

// begin NEXUS definitions

// NEXUS: taken from dmasound.h
#define SND_DEV_DSP16	5	/* Like /dev/dsp but 16 bits/sample */

extern struct file_operations *nexus_audio_fops;
static struct es1370_state *nexus_es1370_state;

// end   NEXUS definitions
...
static int drain_dac1(struct es1370_state *s, int nonblock)
{
	...
	if (s->dma_dac1.mapped || !s->dma_dac1.ready)
		...
		if (signal_pending(current))
                        break;
                if (nonblock) {
//NONEXUS                        remove_wait_queue(&s->dma_dac1.wait, &wait);
...
static int drain_dac2(struct es1370_state *s, int nonblock)
{
	...
	if (s->dma_dac2.mapped || !s->dma_dac2.ready)
		return 0;
//NONEXUS        add_wait_queue(&s->dma_dac2.wait, &wait);
        for (;;) {
		__set_current_state(TASK_INTERRUPTIBLE);
                spin_lock_irqsave(&s->lock, flags);
		...
		if (signal_pending(current))
                        break;
                if (nonblock) {
//NONEXUS                        remove_wait_queue(&s->dma_dac2.wait, &wait);
                        set_current_state(TASK_RUNNING);
                        return -EBUSY;
                }
...
{
	struct es1370_state *s = nexus_es1370_state; //NONEXUS (struct es1370_state *)file->private_data;
	DECLARE_WAITQUEUE(wait, current);
	ssize_t ret = 0;
	unsigned long flags;
	unsigned swptr;
...
//NONEXUS	add_wait_queue(&s->dma_adc.wait, &wait);
	while (count > 0) {
		spin_lock_irqsave(&s->lock, flags);
...
out:
	up(&s->sem);
//NONEXUS        remove_wait_queue(&s->dma_adc.wait, &wait);
	set_current_state(TASK_RUNNING);
	return ret;
}
...
#else
		if (memcpy(s->dma_dac2.rawbuf + swptr, buffer, cnt)) {
#endif
			if (!ret)
				ret = -EFAULT;
			goto out;
...
static unsigned int es1370_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	return mask;
#else
	printk("[es1370] warning: unsupported %s called", __FUNCTION__);
	return 0;
#endif
}
...
// Nexus: dac and midi support are disabled
#ifdef NONEXUS
...
static ssize_t es1370_write_dac(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	...
	if (!access_ok(VERIFY_READ, buffer, count))
		return -EFAULT;
//NONEXUS        add_wait_queue(&s->dma_dac1.wait, &wait);
	while (count > 0) {
		spin_lock_irqsave(&s->lock, flags);
		if (s->dma_dac1.count < 0) {
			s->dma_dac1.count = 0;
...
#else
	if (!pci_register_driver(&es1370_driver, 0)) {
		printk("[audio] es1370 device register failed\n");
		return 1;
	}

	if (es1370_open(NULL, NULL)) {
		printk("[audio] es1370 device open failed\n");
	}

	nexus_audio_fops = &es1370_audio_fops;
	return 0;
#endif
}
