...
int i810_ioctl_nexus(unsigned int cmd, Call_Handle call_handle, 
	...
	case OSS_GETVERSION:
		...
		return IPC_TransferTo(call_handle, destout->desc_num, 
				      (unsigned int)destout->data, 
				      &soundversion, sizeof(int));
	...
	case SNDCTL_DSP_GETBLKSIZE:
		...
		return IPC_TransferTo(call_handle, destout->desc_num, 
				      (unsigned int)destout->data, 
				      (void *)&dmabuf->userfragsize, sizeof(int));
	...
	case SNDCTL_DSP_CHANNELS:
		...
		if (val > 0) {
			...
			return IPC_TransferTo(call_handle, destout->desc_num, 
					      (unsigned int)destout->data, 
					      (void *)&state->card->channels, sizeof(int));
	...
	case SNDCTL_DSP_GETOPTR:
		...
		return IPC_TransferTo(call_handle, destout->desc_num, 
				      (unsigned int)destout->data, 
				      (void *)&cinfo, sizeof(cinfo)) ? -EFAULT : 0;
	...
	case SNDCTL_DSP_GETISPACE:
		...
		return IPC_TransferTo(call_handle, destout->desc_num, 
				      (unsigned int)destout->data, 
				      (void *)&abinfo, sizeof(abinfo))? -EFAULT : 0;
	...
	case SNDCTL_DSP_GETTRIGGER:
		...
		return IPC_TransferTo(call_handle, destout->desc_num, 
				      (unsigned int)destout->data, 
				      (void *)&dmabuf->trigger, sizeof(int));
...
PCI_ProbeFunc pci_probe_func = i810_probe;
