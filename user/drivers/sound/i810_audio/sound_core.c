 ...
 */
//djwill #include <nexus/io_instrument_on.h>
...
#include <linux/fs.h>
//djwill #include <linux/sound.h>
#include <linux/major.h>
//djwill #include <linux/kmod.h>
//djwill #include <linux/devfs_fs_kernel.h>

//djwill #include <nexus/defs.h>
#include <nexus/djwilldbg.h>

#define SOUND_STEP 16


#define DAN_REMOVED 0

//djwill: don't have devfs stuff
typedef unsigned int devfs_handle_t;
#define DEVFS_FL_NONE 0
void devfs_unregister(devfs_handle_t t){ printk("devfs unregister!\n");}
void devfs_unregister_chrdev(int a, char *b){ printk("devfs unregister_chrdev!\n");}
devfs_handle_t devfs_register(devfs_handle_t a1, char *a2, int a3, int a4, int a5, int a6, struct file_operations *a7, char *a8){ printk("devfs unregister!\n"); return 0;}
...
{
	int r;
	struct sound_unit *s=(struct sound_unit *)malloc(sizeof(struct sound_unit));
	char name_buf[16];
...
{
  int dbg = 1;
  printk_djwill("returning 0 for resister_sound_dsp\n");
  return 0;
  /* XXX dsp and all this stuff is not implemented... */
#if 0
...
{
	printk("DAN: %s %s %d", __FUNCTION__, __FILE__, __LINE__);
	return 0;
}
