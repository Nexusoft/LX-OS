...
/* Nexus changes to original Linux kernel 2.4.22 driver
   
   - workaround for strange issue where private member dev->lp seems faulty
     this is consistently alloc()ed kernel mem, so who knows what goes on.
     - hardcoded use of pcnet32_wio functions
     - disabled spinlock
     T find out

   - replaced allocation with nexus page-aligned allocation
     T make sure alignment is correct (some devices automatically add offset)

   - some ifdef NEXUS_UDRIVER to work around differences in 
     Nexus kernel and userspace driver APIs (sigh)
 */
		...
		struct pci_dev *pdev)
{
...
#ifdef NEXUS_UDRIVER
    dev = alloc_etherdev("lance", sizeof(struct pcnet32_private));
#else
    dev = alloc_etherdev(sizeof(struct pcnet32_private));
#endif
    if(!dev)
	return -ENOMEM;
...
#else
	printk("[pcnet32] unsupported deprecated probing triggered\n");
#endif
    }
    ...
    register_netdev(dev);
#else
    nexus_register_netdev(dev, "lance", pcnet32_interrupt, dev);
#endif
...
pcnet32_open(struct net_device *dev)
{
    ...
    val = pcnet32_wio_read_bcr (ioaddr, 2) & ~2;
    if (lp->options & PCNET32_PORT_ASEL)
	val |= 2;
    pcnet32_wio_write_bcr (ioaddr, 2, val);
    
    /* handle full duplex setting */
    if (lp->mii_if.full_duplex) {
	val = pcnet32_wio_read_bcr (ioaddr, 9) & ~3;
	...
	/* workaround of xSeries250, turn on for 79C975 only */
	    i = ((pcnet32_wio_read_csr(ioaddr, 88) | (pcnet32_wio_read_csr(ioaddr,89) << 16)) >> 12) & 0xffff;
	    if (i == 0x2627) val |= 3;
	}
	pcnet32_wio_write_bcr (ioaddr, 9, val);
    }
    
    /* set/reset GPSI bit in test register */
    val = pcnet32_wio_read_csr (ioaddr, 124) & ~0x10;
    if ((lp->options & PCNET32_PORT_PORTSEL) == PCNET32_PORT_GPSI)
	val |= 0x10;
    pcnet32_wio_write_csr (ioaddr, 124, val);
    
    if (lp->mii && !(lp->options & PCNET32_PORT_ASEL)) {
	val = pcnet32_wio_read_bcr (ioaddr, 32) & ~0x38; /* disable Auto Negotiation, set 10Mpbs, HD */
	if (lp->options & PCNET32_PORT_FD)
	    val |= 0x10;
	if (lp->options & PCNET32_PORT_100)
	    val |= 0x08;
	pcnet32_wio_write_bcr (ioaddr, 32, val);
    } else {
	if (lp->options & PCNET32_PORT_ASEL) {  /* enable auto negotiate, setup, disable fd */
		val = pcnet32_wio_read_bcr(ioaddr, 32) & ~0x98;
		val |= 0x20;
		pcnet32_wio_write_bcr(ioaddr, 32, val);
	}
    }
	...
	val = pcnet32_wio_read_csr (ioaddr, 3);
	val |= 0x40;
	pcnet32_wio_write_csr (ioaddr, 3, val);
    }
#endif

    if (lp->ltint) { /* Enable TxDone-intr inhibitor */
	val = pcnet32_wio_read_csr (ioaddr, 5);
	val |= (1<<14);
	pcnet32_wio_write_csr (ioaddr, 5, val);
    }
    ...
    pcnet32_wio_write_csr (ioaddr, 1, (lp->dma_addr + offsetof(struct pcnet32_private, init_block)) &0xffff);
    pcnet32_wio_write_csr (ioaddr, 2, (lp->dma_addr + offsetof(struct pcnet32_private, init_block)) >> 16);

    pcnet32_wio_write_csr (ioaddr, 4, 0x0915);
    pcnet32_wio_write_csr (ioaddr, 0, 0x0001);

    netif_start_queue(dev);

    i = 0;
    while (i++ < 100)
	if (pcnet32_wio_read_csr (ioaddr, 0) & 0x0100)
	    break;
    /* 
     * We used to clear the InitDone bit, 0x0100, here but Mark Stockton
     * reports that doing so triggers a bug in the '974.
     */
    pcnet32_wio_write_csr (ioaddr, 0, 0x0042);

    if (pcnet32_debug > 2)
	printk(KERN_DEBUG "%s: pcnet32 open after %d ticks, init block %#x csr0 %4.4x.\n",
	       dev->name, i, (u32) (lp->dma_addr + offsetof(struct pcnet32_private, init_block)),
	       pcnet32_wio_read_csr(ioaddr, 0));
...
#else
	    if (!(rx_skbuff = lp->rx_skbuff[i] = alloc_skb (PKT_BUF_SZ, 0))) {
#endif
...
#else
		lp->rx_dma_addr[i] = nexuscompat_pci_map_single(lp->pci_dev, rx_skbuff->head, PAGE_SIZE, PCI_DMA_FROMDEVICE);
#endif
...
pcnet32_restart(struct net_device *dev, unsigned int csr0_bits)
{
    ...
    /* ReInit Ring */
    pcnet32_wio_write_csr (ioaddr, 0, 1);
    i = 0;
    while (i++ < 1000)
	if (pcnet32_wio_read_csr (ioaddr, 0) & 0x0100)
	    break;

    pcnet32_wio_write_csr (ioaddr, 0, csr0_bits);
}
...
pcnet32_tx_timeout (struct net_device *dev)
{
	...
	printk(KERN_ERR "%s: transmit timed out, status %4.4x, resetting.\n",
	       dev->name, pcnet32_wio_read_csr(ioaddr, 0));
	pcnet32_wio_write_csr (ioaddr, 0, 0x0004);
	lp->stats.tx_errors++;
	if (pcnet32_debug > 2) {
	    int i;
...
pcnet32_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
...
#else
    lp->tx_dma_addr[entry] = nexuscompat_pci_map_single(lp->pci_dev, skb->head, PAGE_SIZE, PCI_DMA_TODEVICE);
#endif
    ...
    pcnet32_wio_write_csr (ioaddr, 0, 0x0048);
...
// Nexus lock actually seems locked. has to do with lp issue
#ifdef NONEXUS
    spin_lock(&lp->lock);
#endif

    rap = pcnet32_wio_read_rap(ioaddr);
    while ((csr0 = pcnet32_wio_read_csr (ioaddr, 0)) & 0x8600 && --boguscnt >= 0) {
	/* Acknowledge all of the current interrupt sources ASAP. */
	pcnet32_wio_write_csr (ioaddr, 0, csr0 & ~0x004f);

	must_restart = 0;

	if (pcnet32_debug > 5)
	    printk(KERN_DEBUG "%s: interrupt  csr0=%#2.2x new csr=%#2.2x.\n",
		   dev->name, csr0, pcnet32_wio_read_csr (ioaddr, 0));
...
#else
		    free_skb_int(lp->tx_skbuff[entry]);
#endif
...
//#endif
    ...
    pcnet32_wio_write_csr (ioaddr, 0, 0x7940);
    pcnet32_wio_write_rap (ioaddr,rap);
...
// avoid allocation in (kernel) interrupt context
#ifndef NONEXUS
...
pcnet32_rx(struct net_device *dev)
{
	...
	if (status != 0x03) {			/* There was an error. */
...
#else
				nexuscompat_pci_map_single(lp->pci_dev, newskb->head,
...
#else
				     (unsigned char *)(lp->rx_skbuff[entry]->head),
#endif
				     pkt_len,0);
		}
		...
		netif_rx(skb);
#else
		// override default: we know that we've mapped from skb->head onwards
		skb->mac.raw = skb->head;
		skb->len = pkt_len;
#ifdef NEXUS_UDRIVER
		nxnet_dev_rx(skb);
#else
		nxnet_dev_rx_int(skb);
#endif
#endif
		dev->last_rx = jiffies;
		lp->stats.rx_packets++;
...
pcnet32_close(struct net_device *dev)
{
    ...
    lp->stats.rx_missed_errors = pcnet32_wio_read_csr (ioaddr, 112);
     ...
     */
    pcnet32_wio_write_bcr (ioaddr, 20, 4);
...
pcnet32_get_stats(struct net_device *dev)
{
    ...
    spin_lock_irqsave(&lp->lock, flags);
    saved_addr = pcnet32_wio_read_rap(ioaddr);
    lp->stats.rx_missed_errors = pcnet32_wio_read_csr (ioaddr, 112);
    pcnet32_wio_write_rap(ioaddr, saved_addr);
    spin_unlock_irqrestore(&lp->lock, flags);

    return &lp->stats;
}

// Nexus: taken from crc32.c from same Linux version (2.4.22)
#ifndef NONEXUS
...
static void pcnet32_set_multicast_list(struct net_device *dev)
{
    ...
    pcnet32_wio_write_csr (ioaddr, 0, 0x0004); /* Temporarily stop the lance. */
...
static int mdio_read(struct net_device *dev, int phy_id, int reg_num)
{
	...
	phyaddr = pcnet32_wio_read_bcr(ioaddr, 33);

	pcnet32_wio_write_bcr(ioaddr, 33, ((phy_id & 0x1f) << 5) | (reg_num & 0x1f));
	val_out = pcnet32_wio_read_bcr(ioaddr, 34);
	pcnet32_wio_write_bcr(ioaddr, 33, phyaddr);
...
static void mdio_write(struct net_device *dev, int phy_id, int reg_num, int val)
{
	...
	pcnet32_wio_write_bcr(ioaddr, 33, ((phy_id & 0x1f) << 5) | (reg_num & 0x1f));
	pcnet32_wio_write_bcr(ioaddr, 34, val);
	pcnet32_wio_write_bcr(ioaddr, 33, phyaddr);
}
...
#else
	printk("%s.%d not supported in Nexus\n", __FUNCTION__, __LINE__);
#endif
	return -EOPNOTSUPP;
}
...
static int pcnet32_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    int phyaddr = pcnet32_wio_read_bcr (ioaddr, 33);
	...
	case SIOCGMIIREG:		/* Read MII PHY register. */
	    pcnet32_wio_write_bcr (ioaddr, 33, ((data->phy_id & 0x1f) << 5) | (data->reg_num & 0x1f));
	    data->val_out = pcnet32_wio_read_bcr (ioaddr, 34);
	    pcnet32_wio_write_bcr (ioaddr, 33, phyaddr);
	    return 0;
	case SIOCSMIIREG:		/* Write MII PHY register. */
		...
		return -EPERM;
	    pcnet32_wio_write_bcr (ioaddr, 33, ((data->phy_id & 0x1f) << 5) | (data->reg_num & 0x1f));
	    pcnet32_wio_write_bcr (ioaddr, 34, data->val_in);
	    pcnet32_wio_write_bcr (ioaddr, 33, phyaddr);
	    return 0;
	default:
	    return -EOPNOTSUPP;
...
static 
#endif
int __init pcnet32_init_module(void)
{
...
#ifdef NEXUS_UDRIVER
    pci_register_driver(&pcnet32_driver, 0);
#else
