...
__e1000_init_module(void)
{
	...
	ret = pci_module_init(&e1000_driver, 0);
	if(ret >= 0) {
...
module_init(__e1000_init_module);
...
e1000_tx_queue(struct e1000_adapter *adapter, int count, int tx_flags)
{
	...
	/* DAN: this is where the structure describing where the skb
	 * to send (the physical address found by pci_map_single is in
	 * buffer_info->dma) is copied to the device's dma'able memory
	 * (the macro E1000_TX_DESC expands to ring->desc, the vaddr
	 * of the dma'able memory
	 */
	while(count--) {
		buffer_info = &tx_ring->buffer_info[i];
