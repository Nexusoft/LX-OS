...
#error "[e1000] NAPI not supported on Nexus"
#endif
#ifdef NETIF_F_TSO
#error "[e1000] TSO not supported on Nexus"
#endif
#endif

#ifndef __NEXUSKERNEL__
#define printk_red printk

// to maintain integrity and confidentiality, the userspace driver is blind:
// it can read nor write packet contents
//#define NEXUS_E1000_BLIND

// to reduce overhead, replace runtime traps due to memory protection 
// with system calls
#define NEXUS_E1000_PARAVIRT
#endif
...
#ifdef NETIF_F_TSO
#ifndef NONEXUS
#error "TSO not supported"
#endif
#endif

// Nexus equivalent to NAPI
#ifdef __NEXUSKERNEL__

#include <nexus/synch.h>
static Sema nxclean_sema = SEMA_INIT_KILLABLE;

static int
nxclean(struct net_device *netdev)
{
...
static int
nxclean_start(struct net_device *netdev)
{
	nexusthread_fork(nxclean, netdev);
}
...
static
#endif
int __init __e1000_init_module(void)
{
	int ret;
...
#ifdef NEXUS_UDRIVER
	ret = pci_register_driver(&e1000_driver, 0);
#else
...
mdelay(20);	// NXDEBUG: infrequent race with DDRM
...
#ifdef NEXUS_UDRIVER
	netdev = alloc_etherdev("e1000", sizeof(struct e1000_adapter));
#else
...
#if defined NONEXUS || defined NEXUS_UDRIVER
	register_netdev(netdev);
#else
	nexus_register_netdev(netdev, "e1000", e1000_intr, netdev);
#endif
...
#ifdef __NEXUSKERNEL__
	txdr->desc = nexuscompat_pci_alloc_consistent(pdev, txdr->size, &txdr->dma);
#else
#ifdef NEXUS_E1000_BLIND
	// trap all writes, to be able to inspect descriptors 
	// to point only to allowed addresses
	//txdr->desc = Device_mem_alloc(txdr->size >> PAGE_SHIFT, &txdr->dma);
	txdr->desc = pci_alloc_consistent(pdev, txdr->size, &txdr->dma);
	printk("NXDEBUG: XXX reenable protected txring\n");
#else
	txdr->desc = pci_alloc_consistent(pdev, txdr->size, &txdr->dma);
#endif
#endif
...
#ifdef __NEXUSKERNEL__
	rxdr->desc = nexuscompat_pci_alloc_consistent(pdev, rxdr->size, &rxdr->dma);
#else
...
#ifndef NDEBUG
	printk_red("[e1000] tx stall detected\n");
#endif
...
// This version of the e1000 driver did not support full IP + TCP offload
// I had to implement it manually according to HW spec. All necessary
// changes are encapsulated in NEXUS_CSUM ifdefs
#define NEXUS_CSUM
...
e1000_tx_csum(struct e1000_adapter *adapter, struct sk_buff *skb)
{
	...
	if(skb->ip_summed == CHECKSUM_HW) {
		...
		int ret = 0;	// communicate whether data descriptor should set IP and possibly TCP offload

		// no segmentation for any protocol
		...
		if (skb->protocol) {
			...
			else if (skb->protocol == 17) {
				csum_off = 6;
				ret = 1;
			}
			
			context_desc->upper_setup.tcp_fields.tucss = 34; // NB: does not support IP options with TCP/UDP
			context_desc->upper_setup.tcp_fields.tucso = 34 + csum_off;
...
#else
#ifdef NEXUS_CSUM
		if (skb->mac.ethernet->h_proto == cpu_to_be16(ETH_P_IP)) {
			struct iphdr *iph;
			int cmd, transport_off, csum_off;
			
			iph = (void *) skb->head + 14;
			transport_off  = 14 + (4 * iph->ihl);	// offset from start of packet
			csum_off = 0;				// offset from start of TRANSPORT header
			...
			// note: according to hw spec, these MUST be set even if offloading is disabled
			context_desc->upper_setup.tcp_fields.tucss = transport_off;
			context_desc->upper_setup.tcp_fields.tucso = transport_off + csum_off;
			context_desc->upper_setup.tcp_fields.tucse = 0;
			context_desc->cmd_and_length = cpu_to_le32(cmd);
		}
		else {
	...
	unsigned int first)
{
	...
	while(len) {
		...
		buffer_info->dma =
#ifdef NEXUS_E1000_BLIND
			(unsigned long) skb->real_dev;
		if (!buffer_info->dma) {
			printk("[e1000] BUG: no paddr in tx\n");
			assert(0);
		}
#else
#ifdef NONEXUS
...
#else
			nexuscompat_pci_map_single(adapter->pdev,
#endif
				skb->data + offset,
				size,
				PCI_DMA_TODEVICE);
...
#else
		// no scatter-gather in nexus
		printk_red("Panic at %s.%d\n", __FUNCTION__, __LINE__);
		assert(0);
#endif
	}
...
#if 0 //def NEXUS_E1000_BLIND
		// in blind mode, refmon only allows complete 16B writes,
		// to be able to check safety of embedded address
		struct { uint64_t addr; uint32_t lower; uint32_t upper; } x;
		assert(sizeof(x) == 16);
		x.addr  = cpu_to_le64(buffer_info->dma);
		x.lower = cpu_to_le32(txd_lower | buffer_info->length);
		x.upper = cpu_to_le32(txd_upper);
		if (Device_mem_write(tx_desc, sizeof(x), &x)) {
			printk("[e1000] ERROR: txdesc write failed\n");
			abort();
		}
#else
...
#ifdef NEXUS_E1000_PARAVIRT
	Device_mem_write(adapter->hw.hw_addr + E1000_TDT, 4, i);
#else
...
e1000_xmit_frame(struct sk_buff *skb, struct net_device *netdev)
{
	...
	if(E1000_DESC_UNUSED(&adapter->tx_ring) < DESC_NEEDED) {
		...
		netif_stop_queue(netdev);
#else
		printk_red("[e1000] WARNING: TX overflow. Should quiesce input\n");
#endif
		return 1;
	}
...
#else
			printk_red("[e1000] WARNING: 82547 stall\n");
#endif
			return 1;
		}
	}
...
#ifdef __NEXUSKERNEL__
	// must handle outside interrupt context: our NAPI
	V(&nxclean_sema);
#else
#ifdef CONFIG_E1000_NAPI
...
#else
	// clean in nexus userspace: no real interrupt handler, 
	// so no need for separate interrupt thread
...
#else
		printk_red("NXDEBUG: rx_complete not implemented\n");
#endif
		e1000_irq_enable(adapter);
	}
...
#endif
{
	...
	while(rx_desc->status & E1000_RXD_STAT_DD) {
...
#else
      		skb_put(skb, length - ETHERNET_FCS_SIZE + ENET_HEADER_SIZE);
#endif
		...
		if(adapter->vlgrp && (rx_desc->status & E1000_RXD_STAT_VP)) {
			...
			netif_rx(skb);
#else
			skb->len = length - ETHERNET_FCS_SIZE + ENET_HEADER_SIZE;
			nxnet_dev_rx(skb);
#endif
		}
...
#ifdef NEXUS_E1000_BLIND
		skb = alloc_skb_ex(adapter->rx_buffer_len + reserve_len, 0, &buffer_info->dma);
#else
#ifndef __NEXUSKERNEL__
		skb = dev_alloc_skb(adapter->rx_buffer_len + reserve_len);
#else
		skb = alloc_skb(adapter->rx_buffer_len + reserve_len, 0);
#endif
#endif
...
// cannot touch skb if blind
#ifndef NEXUS_E1000_BLIND
// disabled because nexus expects eth at start of page. alignment is no biggie
#ifdef NONEXUS
...
#else
		skb->data = skb->head; // ensure data is written at the start
#endif
...
#ifdef NEXUS_E1000_BLIND
		// user driver: already set by alloc_skb_ex
#else
		buffer_info->dma =
#ifdef NONEXUS
			pci_map_single(pdev,
#else
			nexuscompat_pci_map_single(pdev,
#endif
			               skb->data,
...
#ifdef NEXUS_E1000_PARAVIRT
			Device_mem_write(adapter->hw.hw_addr + E1000_RDT, 4, i);
#else
