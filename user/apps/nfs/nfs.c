...
u32 *xdr_encode_sattr_len(u32 *p, int len) {
	p = xdr_encode_int(p, 0666); // mode : don't change
...
u32 *xdr_encode_sattr_mode(u32 *p) {
	p = xdr_encode_int(p, 1); p = xdr_encode_int(p, 0666); // mode
	p = xdr_encode_int(p, 0); // uid: don't change
	p = xdr_encode_int(p, 0); // gid: don't change
...
#define EXPECT_STATUS_NOERR(val) do { \
		if (len < 4) QUIETFAIL(); \
		len -= 4; \
		u32 r_val; \
		p = xdr_decode_int(p, &r_val); \
		if (r_val != (val)) QUIETFAIL(); \
	} while (0);
...
int nfs_init(char *serverip, char *mountpath, char *localmachinename, unsigned int euid) {
	...
	printf("[nfs] attempting %s:%s\n", serverip, mountpath);
	printf("      with linux uid: %d\n", euid);
...
struct nfs_node *make_node(struct nfs_node *node, struct nfs_node *parent, 
			   char *name, int have_attr) 
{
	struct nfs_node *ret_node;
	
	if (!have_attr && nfs_get_attr(node) < 0) {
		printf("MKNODE error in attr\n");
		return NULL;
	}
       
	ret_node = malloc(sizeof(struct nfs_node));
