...
void fatfs_fat_init(struct fatfs *fs)
{
	...
	for (i=0;i<FAT_BUFFERED_SECTORS;i++)
	{
		...
		// Make sure the buffer is mem aligned
		if (!fs->fat_buffers[i].sector) {
			fs->fat_buffers[i].sector = memalign(FAT_SECTOR_SIZE, FAT_SECTOR_SIZE);
			assert(fs->fat_buffers[i].sector);
		}
		memset(fs->fat_buffers[i].sector, 0x00, FAT_SECTOR_SIZE);
...
UINT32 fatfs_find_next_cluster(struct fatfs *fs, UINT32 current_cluster)
{
	...
	if (nextcluster==0x0FFFFFFF || 
                (current_cluster == fs->rootdir_first_cluster && nextcluster >= 0x0FFFFFF8)) 
		return (FAT32_LAST_CLUSTER); 
	else 
