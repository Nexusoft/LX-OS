                           ...
                           unsigned shadow_mode_enabled)
{
    ...
    nl_munmap(vl1tab, PAGE_SIZE);
    nl_munmap(vl2tab, PAGE_SIZE);
    return 0;
...
static int setup_guest(int xc_handle,
                       ...
                       unsigned int cons_evtchn, unsigned long *cons_mfn,
                       uint32_t required_features[XENFEAT_NR_SUBMAPS])
{
    ...
    *cons_mfn = page_array[2];
    ...
    start_info->console.domU.evtchn = cons_evtchn;
    ...
    nl_munmap(start_info, PAGE_SIZE);
    ...
    nl_munmap(shared_info, PAGE_SIZE);
                       ...
                       uint32_t required_features[XENFEAT_NR_SUBMAPS])
{
    ...
    unsigned long vpt_end;
    unsigned long v_end;
    unsigned long guest_store_mfn, guest_cons_mfn, guest_shared_info_mfn;
...
#define NR(_l,_h,_s)                                                    \
...
#else
        PERROR("No grant tables in Nexus Xen");
#endif
    }
    else
    {
    ...
    *cons_mfn = page_array[(vcons_start-dsi.v_start) >> PAGE_SHIFT];
    if ( xc_clear_domain_page(xc_handle, dom, *store_mfn) ||
         xc_clear_domain_page(xc_handle, dom, *cons_mfn) )
        goto error_out;
    if ( shadow_mode_enabled )
    {
        guest_store_mfn = (vstoreinfo_start-dsi.v_start) >> PAGE_SHIFT;
        guest_cons_mfn = (vcons_start-dsi.v_start) >> PAGE_SHIFT;
    }
    else
    {
        guest_store_mfn = *store_mfn;
        guest_cons_mfn = *cons_mfn;
    }
    ...
    start_info->store_evtchn = store_evtchn;
    start_info->console.domU.mfn   = guest_cons_mfn;
    start_info->console.domU.evtchn = cons_evtchn;
    if ( initrd->len != 0 )
    {
    ...
    if ( xc_finish_mmu_updates(xc_handle, mmu) )
        goto error_out;
    if(!sanity_check_m2p_consistency()) {
        assert(0);
    }
...
static int xc_linux_build_internal(int xc_handle,
                                   ...
                                   unsigned int cons_evtchn,
                                   unsigned long *cons_mfn)
{
