...
long HYPERCALL_console_io(int cmd, int count, char *buffer) {
    ...
    case CONSOLEIO_read: {
        ...
        int returned_count = Console_GetData(desc, count);
        if(returned_count < 0) {
    ...
    case CONSOLEIO_setmode: {
        ...
        switch(mode) {
        case CONSOLEIO_RAW:
            rv = Console_SetInputMode(KBD_RAW);
            if(rv != 0) {
	...
	printf("[xen] hypercall consoleio/getkbent not supported\n"); 
        return -EACCES;
    }
    default:
...
long HYPERCALL_getNexusVariables(struct NexusVariables *variables) {
    ...
    struct NexusVariables var = {
        .print_handle = 0,	/* deprecated */
        .mouse_handle = 0,	/* deprecated */
    };
...
int HYPERCALL_VNet_Init(int vnic_num, char *assigned_mac) {
    ...
    printf("[xen] WARNING: unimplemented %s called\n", __FUNCTION__);
    return -SC_NOTASSIGNED;
}
