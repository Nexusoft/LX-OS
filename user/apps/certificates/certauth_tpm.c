/** NexusOS: certificate authority on TPMs

    Generates a certificate that a key (the 'Attestation Identity Key', or AIK) 
    speaksfor a TPM

    Returns an X509 certificate with the meaning
    ``TPM Privacy CA says AIK speaksfor a Trusted Platform Module''
 */
...
RSA *check_idbinding(IdentityReqData *req) {
  ...
  int ret = RSA_verify(NID_sha1, hash, TPM_HASH_SIZE,
