/** NexusOS: Generic credential creation function */
...
int generate_credential(X509 *new, RSA *pubkey, RSA *privkey,
  ...
  if(!X509_NAME_add_entry_by_txt(issuer_name, "O", chtype, (unsigned char *)"Cornell University", -1, -1, 0))
    FAIL("Could not add entry O\n!");
  if(!X509_NAME_add_entry_by_txt(issuer_name, "OU", chtype, (unsigned char *)"Nexus Operating System", -1, -1, 0))
  ...
  if(subject != NULL){
    ...
    if(!X509_NAME_add_entry_by_txt(subject_name, "L", chtype, (unsigned char *)"Ithaca", -1, -1, 0))
      FAIL("Could not add entry L\n!");
    if(!X509_NAME_add_entry_by_txt(subject_name, "O", chtype, (unsigned char *)"Cornell University", -1, -1, 0))
      FAIL("Could not add entry O\n!");
    if(!X509_NAME_add_entry_by_txt(subject_name, "OU", chtype, (unsigned char *)"Nexus Operating System", -1, -1, 0))
      FAIL("Could not add entry OU\n!");
