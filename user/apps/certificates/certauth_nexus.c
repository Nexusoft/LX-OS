/** NexusOS: certificate authority on Nexus installations

    Generates a certificate that a key (the 'Nexus Signing Key', or NSK) 
    speaksfor a trusted Nexus installation

    Returns an X509 certificate with the meaning
    ``Nexus Vetting CA says NSK speaksfor a Trusted Nexus''

    AND 

    Returns a NAL statement signed with the NSK that says
    ``pcrs(der(NSK)) = { PCR0, PCR1, .., PCR7 }''
 */
...
X509 *handle_request(unsigned char *certifybuf, int certifysize, 
  ...
  unsigned char hash2[TPM_HASH_SIZE];
  ...
  printf("\n");
  if(RSA_verify(NID_sha1, hash, TPM_HASH_SIZE,
...
void * handle_request2(unsigned char *certifybuf, int certifysize, 
			       unsigned char *aikbuf, int aiksize) {
...
#else
  fprintf(stderr, "XXX extract public key from RSA key");
  exit(1);
#endif
...
#else
  fprintf(stderr, "XXX sign formula with private key");
  return der;
#endif
