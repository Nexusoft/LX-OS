...
int blockingsslread(SSL *sock, void *ptr, int len){
  int left, tot = len;
  for(;len > 0;){
    left = SSL_read(sock, ptr, len);
    
    if(left == 0){
      return -1; //socket aborted
    }
...
void Minisslsocket::reader_thread(){
  ...
  while(sock >= 0){
    ...
    if((err = 
#ifdef DISABLE_SSL
      blockingread(sock, len, sizeof(uint32_t) *2)
#else
      blockingsslread(conn, len, sizeof(uint32_t) *2)
#endif
    ) != sizeof(uint32_t)*2){ 
    ...
    if(len[0] != len[1]){  
      ...
      ERR_print_errors_fp(stderr);
#endif
      printf("invalid length read : %d(%d):%d(%d)\n", len[0], (int)sizeof(len[0]), len[1], (int)sizeof(len[0])); 
      break; 
    }
    ...
    if((err = 
#ifdef DISABLE_SSL
      blockingread(sock, data, len[0])
#else
      blockingsslread(conn, data, len[0])
#endif
    ) != (int)len[0]){ 
