...
Minipipe::~Minipipe(){
  ...
  for(curr = trash; curr != NULL; curr = tmp){
    tmp = curr->next;
    safe_free(curr->data);
    safe_free(curr);
  }
...
void Minipipe::write_malloced(char *data, int len){
  ...
  struct Minipipe_v *tmp = get_entry();
...
void Minipipe::dispose_entry(struct Minipipe_v *entry){
  lock();
  {
    entry->next = trash;
    trash = entry;
...
struct Minipipe_v *Minipipe::get_entry(){
  struct Minipipe_v *ret;
  lock();
  {
    if(trash == NULL){
      ret = (struct Minipipe_v *)safe_malloc(sizeof(struct Minipipe_v));
    } else {
      ret = trash;
      trash = trash->next;
    }
