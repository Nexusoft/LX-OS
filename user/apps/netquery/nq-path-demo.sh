...
. ${ROOT}/setup-switch-net-vm.sh

function open_window ()
{
    cmd="$@ ; cat > /dev/null"
    echo "Open window: $cmd" 
    xterm -sl 1000 -hold -sk -si -fn lucidasanstypewriter-10 -e "$@" &
...
## Nexus TCP firewall
devs=$( echo $INNER | perl -e 'print join(" ", map { "-d $_" } split(" ",<STDIN>))' );

echo "Devs: $devs"
#exit

open_window ${ROOT}/switchd -f -h 128.84.227.11 -p 9000 $devs TAP 128.84.227.11 8069 128.84.227.8 128.84.227.11
...
sleep 4
# Default: Accept TPM (as of 4/17/09), accept all TCP
...
# accept all
open_window ${ROOT}/switch-enforcer -h 128.84.227.11 -p 9000 -a > /tmp/switch-enforcer.out 2> /tmp/switch-enforcer.err &
# reject all
