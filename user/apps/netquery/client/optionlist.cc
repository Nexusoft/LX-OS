...
char *Command_List::process_command(char *command){
  char *option, *value = NULL, *ctx, *cmd_str, *sep = " \t\n\r";
  ...
  //  printf("processing options (remaining:%s)\n", ctx);
  for(option = ctx?strsep(&ctx, sep):ctx; option != NULL; option = ctx?strsep(&ctx, sep):ctx){
    if(option == NULL){
      break;
    }
    ...
    //printf("((%x:%x:%x))", opt_flags, opt_flags&OPTION_TYPEMASK, OPTION_TYPE_NULL);
