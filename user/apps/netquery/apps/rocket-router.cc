...
#define INVALID_VERTEX INT_MAX
  ...
  void build_pop_transaction_map(POPTransactionMap &output_map) {
	...
	exit(-1);
      ...
      output_map[pop->first] = new Transaction(trust_all, trust_attrval_all, p);
    }
  }
...
struct ForwardingEntry {
  ...
  bool operator<(const ForwardingEntry &f) const {
    if(ip_prefix == f.ip_prefix) {
      cerr << "multicast forwarding tables not supported!\n";
    }
...
struct ForwardingTable : vector<ForwardingEntry> {
...
#define INIT()					\
    ...
    INIT()
  {
    ...
    for(i=0; i < prefix.internal_neighbors.size(); i++) {
      interfaces.push_back(new Interface(*this, interfaces.size(), prefix.internal_neighbors[i]));
    }
    for(i=0; i < prefix.external_neighbors.size(); i++) {
      interfaces.push_back(new Interface(*this, interfaces.size(), prefix.external_neighbors[i]));
    }
  ...
  void tspace_write_fib(Transaction &t, 
    ...
    T_SwitchFabric *t_fabric = this->fabric.load(t);
...
void Topology::init_tuplespace(const string &tid_ofname) {
  ...
  if((int)router_map.size() != total_written_count) {
  ...
  for(POPTransactionMap::iterator i = pop_tr_map.begin();
      i != pop_tr_map.end(); ++i) {
    cerr << "Wrote " << pop_written_count[i->first] << " to " << i->first << "\n";
  ...
  Transaction *t = new Transaction(trust_all, trust_attrval_all, &NQ_default_owner);
...
void Topology::tspace_update_fib(Topology::InterPOPGraph *pop_graph) {
  ...
  if(commit_mode == ONE_BIG_COMMIT) {
    t = new Transaction(trust_all, trust_attrval_all, &NQ_default_owner);
  }
#endif
...
Topology *load_topology(const string &topo_fname, const DistanceDB &distances) {
  ...
  for( ; lines != end; lines++) {
    ...
    parse_external_ips:
      RocketPrefix prefix = RocketPrefix(curr_line);
      if(prefix.valid) {
	cerr << "found valid after first invalid prefix???\n";
      }
      // for now, don't need the external ips
    }
  }
...
int Topology::split_disconnected_pops(void) {
  ...
  for(tie(vi,v_end) = vertices(*inter_graph); vi != v_end; vi++) {
    ...
    if(components.size() > 1) {
      ...
      cerr << "Node " << *vi << "(" << pop_location << ") has " << components.size() << " components\n";
...
int main(int argc, char **argv) {
  ...
  if(argc < 4) {
    cout << "did not specify file names\n";
    exit(-1);
  }

  int opt;
  while( (opt = getopt(argc, argv, "s:vn:S:RO:D")) != -1) {
    switch(opt) {
    case 'n':
  ...
  case 0: {
	...
	i != all_locs.end(); i++) {
      pop_file << i->first << " " << gethostname() << " "<< pop_port_num++ << "\n";
    }
    pop_file.close();
    break;
  }
