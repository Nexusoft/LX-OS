...
static int parse_serialnum(SerialNum *num, unsigned char *buf){
  ...
  num->num = (unsigned char *)malloc(num->len);
...
static int parse_name(Name *name, unsigned char *buf){
  ...
  name->data = (unsigned char *)malloc(name->datalen);
...
static int parse_validity(Validity *validity, unsigned char *buf){
  ...
  validity->starttime = (char *)malloc(len + 1);
  ...
  validity->endtime = (char *)malloc(len + 1);
...
static int parse_bit_string(BitString *bs, unsigned char *buf){
  ...
  bs->data = (unsigned char *)malloc(bs->datalen);
...
static int parse_int(int *numlen, unsigned char **num, unsigned char *buf){
  ...
  *num = (unsigned char *)malloc(*numlen);
...
static int parse_tbs_certificate(TBSCertificate *tbs, unsigned char *buf){
  ...
  tbs->data = (unsigned char *)malloc(tbs->datalen);
...
ParsedX509 *parse_x509(unsigned char *buf, int buflen){
  ...
  ParsedX509 *x509 = malloc(sizeof(ParsedX509));
...
int parse_subjaltname_internals(char **dns, char **uri, unsigned int *ip, int maxlen, unsigned char *buf){
  int len;
  int off = 0;
  char *target;
  while(off < maxlen){
