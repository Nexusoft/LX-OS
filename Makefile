...
IDLGEN		:= $(NEXUSROOT)/tools/idlgen/nexus-idl/nexus-idlgen
...
$(USER_INCL)/linux: $(BUILDROOT)
	...
	@(cd build/user/include; tar xjf $(NEXUSROOT)/common/packages/linux-headers/linux-headers-$(LINUXVERSION).tbz2)

$(BUILDROOT)/kernel/include/linux: $(BUILDROOT)
	@mkdir -p build/kernel/include
	@(cd build/kernel/include; tar xjf $(NEXUSROOT)/common/packages/linux-headers/linux-headers-$(LINUXVERSION)-kernel.tbz2)

$(BUILDROOT)/kernel/drivers/include/linux: $(BUILDROOT)
	@mkdir -p build/kernel/drivers/include
	@(cd build/kernel/drivers/include; tar xjf $(NEXUSROOT)/common/packages/linux-headers/linux-headers-$(LINUXVERSION)-kdrivers.tbz2)
...
$(USER_LIBS)/libc.a:
	@echo "Building libc"
	@make -C user/packages/uclibc 		$(IOREDIR)
	@make -C user/packages/uclibc install 	$(IOREDIR)

$(USER_LIBS)/libcrypto.a:
	@echo "Building openssl"
	@make -C user/packages/openssl 		$(IOREDIR)
	@make -C user/packages/openssl install 	$(IOREDIR)
