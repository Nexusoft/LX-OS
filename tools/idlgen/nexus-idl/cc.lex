...
"service"	return tok(TOK_SERVICE);
"syscall"	return tok(TOK_SYSCALL);
"includefiles"	return tok(TOK_INCLUDEFILES);
"secure_channel"	return tok(TOK_SECURECHANNEL);

"interface" return tok(TOK_INTERFACE);
"__caller__" return tok(TOK_CALLER);
"__callee__" return tok(TOK_CALLEE);
"lf_generate" return tok(TOK_GENERATE);
"lf_check" return tok(TOK_CHECK);
"decls" return tok(TOK_DECLS);

"__output__" return tok(TOK_OUTPUT);
"__input__" return tok(TOK_INPUT);
"__builtin__" return tok(TOK_BUILTIN);

"__interpose_on__" return tok(TOK_INTERPOSE_ON);
"__implements__" return tok(TOK_IMPLEMENTS);
"__interpose__" return tok(TOK_INTERPOSE);
"__Call__" return tok(TOK_CALL);
"__Transfer__" return tok(TOK_TRANSFER);

"__NoGenReturn__" return tok(TOK_NOGENRETURN);
"__NoGenErrno__" return tok(TOK_NOGENERRNO);
"__BareCall__" return tok(TOK_BARECALL);
