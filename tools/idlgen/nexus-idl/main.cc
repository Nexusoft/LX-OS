  ...
  */
  ...
  string gen_IPC_Return(bool is_syscall, 
    ...
    return 
      ...
      "	} else {\n"
      "		IPC_TransferTo(" & CALL_HANDLE & ", RESULT_DESCNUM, " &
      "DESCRIPTOR_START, " & RESULT & ", " & RESULT_LEN & ");\n" &
...
private:
  void generateGeneric(GenericIDL *syscall_decl, string errno_name,
	...
	*server << "if(!IS_ASYNC_PROCESSORTYPE(type)) {\n";
	{
	  int i;
	  for(i=0; i < 2; i++) {
	    string funcname;
	    ...
	    switch(i) {
	    case 0:
	      *server << "	if(type == SERVERPROCESSOR_SYNC) {\n";
	      funcname = "IPC_RecvCall";
	      break;
	    case 1:
	      *server << 
		"	else {\n"
		"		assert(type == SERVERPROCESSOR_SYNC_FORK);\n";
	      funcname = "IPC_RecvCallAndFork";
	      last_arg = ", data.is_forked";
	      break;
	    }
	    *server <<
	      "		if(data.is_forked != NULL) memset(data.is_forked, 0, sizeof(*data.is_forked));\n" <<
	      "		call_result = " << funcname << "(service_port_handle, dataBuf, &dataLen, &cdesc" << last_arg << ");\n" <<
	      call_tail <<
	      "	}\n";
	  }
	  *server << 
	    "	/* If forked, then the caller thread is is in the is_forked structure */\n" <<
	    "   	if(data.is_forked != NULL && data.is_forked->is_forked) {\n" <<
	    "		call_handle = data.is_forked->call_handle;\n" <<
	    "	}\n";
	  *server << 
	    "	if(call_result < 0) {\n" <<
	    "		return call_result;\n" <<
	...
	// SERVER_PROCESSOR_ASYNC
	*server << "} else if(IS_ASYNC_PROCESSORTYPE(type)) {\n";
	{
	  *server << "	AsyncReceiveInfo info = IPC_AsyncReceive(" << service_port_handle() << ", dataBuf, dataLen, NULL);\n"
	    "	ipd_id = info.ipd_id;\n"
	    "	call_handle = info.call_handle;\n"
	    "	dataLen = info.message_len;\n";
	}
	...
	*server << "} else {\n"
      ...
      *server << "		return -1;\n";
      *server << "	}\n";
      *server << "	if(IPC_TransferFrom(call_handle, message.desc_num, dataBuf, (unsigned int)message.data, dataLen) != 0) {\n";
  ...
  void postvisitServiceIDL(ServiceDecl *syscall_decl) {
    ...
    *server << "\n\n // Processors and initialization\n";

    {
      ...
      if(!generateInterpose) {
	serverConstructorBody << "	" << service_port_handle() << " = IPC_CreatePort( &" << service_server_port_num() << " );\n";
      }
      assert(!registerName);
