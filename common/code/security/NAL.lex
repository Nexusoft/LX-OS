...
#define yyerror(msg) yyerror_localized(yyscanner, msg, yylloc, NULL, NULL)
...
<QSTR>{
	...
	\n { yyerror("unterminated string constant"); yyterminate(); }
	<<EOF>> { yyerror( "unterminated string constant" ); yyterminate(); }
	\\[0-7]{1,3} {
		int res = strtol(yytext+1, 0, 8);
		if (res > 0xff) {
			yyerror("invalid string escape sequence");
			yyterminate();
		}
		str_add((char)res);
	}
	\\[0-9]+ { yyerror("invalid string escape sequence"); yyterminate(); }
	\\n { str_add('\n'); }
	\\t { str_add('\t'); }
	\\r { str_add('\r'); }
	\\b { str_add('\b'); }
	...
	. { yyerror( "invalid character in string constant" ); yyterminate(); }
}
...
0[0-7]* { 
	...
	if (res == INT_MIN && strcmp(yytext, "020000000000")) {
		yyerror("underflow in octal constant");
		yyterminate();
	}
	if (res == INT_MAX && strcmp(yytext, "017777777777")) {
		yyerror("overflow in octal constant");
		yyterminate();
	}
...
0x[0-9a-fA-F]* { 
	...
	if (res == INT_MIN && strcmp(yytext, "0x80000000")) {
		yyerror("underflow in hex constant");
		yyterminate();
	}
	if (res == INT_MAX &&
		(strncmp(yytext, "0x7", 3) || strlen(yytext) != 10 || strspn(yytext+3, "fF") != 7)) {
		yyerror("overflow in hex constant");
		yyterminate();
	}
	*yylval = form_newval(F_TERM_INT, res);
	return INTEGER;
...
[+-]{0,1}[1-9][0-9]* {
	...
	if (res == INT_MIN && strcmp(yytext, "-2147483648")) {
		yyerror("underflow in decimal constant");
		yyterminate();
	}
	if (res == INT_MAX && strcmp(yytext, "2147483647")) {
		yyerror("overflow in decimal constant");
		yyterminate();
	}
	*yylval = form_newval(F_TERM_INT, res);
	return INTEGER;
...
<BYTESDATA>{
	...
	<<EOF>> { yyerror( "unterminated byte array constant" ); yyterminate(); }
	[0-9a-fA-F]{2} {
		int res = strtol(yytext, 0, 16);
		str_add((char)res);
	}
	[ \n\t\f\v\r]+ { }
	. { yyerror( "invalid character in byte array constant" ); yyterminate(); }
}
