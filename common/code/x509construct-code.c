...
// kwalsh: some of this could be merged with / replaced by the code in der.[ch]
...
static int construct_length(int len, unsigned char *buf, int buflen){
  ...
  if(len <= 0xff)
    ...
    numoctets = 3;
  else if(len <= 0xffffffff) // kwalsh: this code looks broken (a signed/unsigned error) 
    numoctets = 4;
  else{
...
static inline int construct_boolean(int is_true, unsigned char *buf, int buflen){
  int written = 0;
  if(TLEN > 0)
    buf[TLEN -1] = is_true ? 0xff : 0x00;
  written += 1;

  written += construct_length(1, buf, TLEN);

  if(TLEN > 0)
    buf[TLEN -1] = TYPE_BOOLEAN;
  written += 1;
...
static inline int construct_octet_string(unsigned char *octets, int len, unsigned char *buf, int buflen){
  int written = 0;
  if(TLEN >= len)
    memcpy(&buf[TLEN - len], octets, len);
  written += len;

  written += construct_length(len, buf, TLEN);

  if(TLEN > 0)
    buf[TLEN -1] = TYPE_OCTET_STRING;
  written += 1;
...
static int construct_version(int version, unsigned char *buf, int buflen){
  ...
  case 1:
    if(CONSTRUCT_VERSION_EXPLICITLY){
      if(TLEN >= VERSIONLEN)
	memcpy(&buf[TLEN - VERSIONLEN], VERSION1, VERSIONLEN);
      written += VERSIONLEN;
    }
    else
  ...
  case 2:
    if(TLEN >= VERSIONLEN)
      memcpy(&buf[TLEN - VERSIONLEN], VERSION2, VERSIONLEN);
    written += VERSIONLEN;
    break;
  case 3:
    if(TLEN >= VERSIONLEN)
      memcpy(&buf[TLEN - VERSIONLEN], VERSION3, VERSIONLEN);
    written += VERSIONLEN;
    break;
  };
...
static int construct_extensions(ToBuildTBS *tbs, unsigned char *buf, int buflen) {
  int written = 0;
  // no path length constraint
  // IS_CA
  unsigned char is_ca[] = {0x30, 0x03, 0x01, 0x01, 0xff};
  written += construct_octet_string(is_ca, sizeof(is_ca), buf, TLEN);
  written += construct_boolean(1, buf, TLEN); // critical
  written += construct_object(OBJID_basicConstraints, sizeof(OBJID_basicConstraints), buf, TLEN);

  written += construct_sequence(written, CLASS_UNIVERSAL, SEQ_CONSTRUCTED, buf, TLEN); // finish sequence for basicConstraints

  written += construct_sequence(written, CLASS_UNIVERSAL, SEQ_CONSTRUCTED, buf, TLEN); // finish sequence for extensions
  // extension 
  written += construct_length(written, buf, TLEN);
  if(TLEN > 0) {
    buf[TLEN - 1] = 0xa3; // context [3] = extensions
  }
  written += 1;
  return written;
}
...
static int construct_tbs_certificate(ToBuildTBS *tbs, unsigned char *buf, int buflen){
  ...
  if(tbs->version == 3) {
    ...
    written += construct_extensions(tbs, buf, TLEN);
  }
...
int construct_x509(unsigned char *serialnum, int serialnumlen,
  ...
  tobuild.iss_countryname = iss_countryname;
  tobuild.iss_statename = iss_statename;
  tobuild.iss_localityname = iss_localityname;
  tobuild.iss_orgname = iss_orgname;
  tobuild.iss_orgunit = iss_orgunit;
  tobuild.iss_commonname = iss_commonname;
  ...
  tobuild.subj_countryname = subj_countryname;
  tobuild.subj_statename = subj_statename;
  tobuild.subj_localityname = subj_localityname;
  tobuild.subj_orgname = subj_orgname;
  tobuild.subj_orgunit = subj_orgunit;
  tobuild.subj_commonname = subj_commonname;
