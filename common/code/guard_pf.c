...
void judge_add(Judge *f, Form *h) {
  int i, n = PointerVector_len(&f->hyp);
  for (i = 0; i < n; i++) {
...
int judge_print_graphviz0(FILE *out, Judge *f, int *num_nodes, PointerVector *givens, PointerVector *assumptions, PointerVector *conclusions) {
  ...
  if (conclusions) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (!judge_cmp(f, g)) {
	fprintf(out, "/* eliminating dup: %d %d */\n", f->was_dup, g->was_dup);
	return i+1;
      }
    }
...
int judge_print_dhtml0(FILE *out, Judge *f, int *num_nodes, PointerVector *givens, PointerVector *assumptions, PointerVector *replnames, PointerVector *replvals) {
  ...
  *e = '\0';
  ...
  if (f->comment) {
    fprintf(out, " // comment: %s", f->comment);
  }
  fprintf(out, "\n");
...
void judge_print_dhtml(FILE *out, Judge *f, char *name, PointerVector *replnames, PointerVector *replvals) {
  PointerVector givens, assumptions;
  PointerVector_init(&givens, 16, POINTERVECTOR_ORDER_PRESERVING);
  PointerVector_init(&assumptions, 16, POINTERVECTOR_ORDER_PRESERVING);
