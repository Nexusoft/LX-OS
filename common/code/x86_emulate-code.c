    ...
    struct x86_emulate_ops  *ops)
{
    ...
    struct cpu_user_regs_disas _regs = *ctxt->regs;
...
#endif
    default:
  	printk("pagefault handler: unknown mode\n");
        return -1;
    }
 ...
 done_prefixes:
    ...
    case 0x63: /* movsxd */
        if ( mode != X86EMUL_MODE_PROT64 ) {
            printk("movsxd unsupported mode\n");
	    goto cannot_emulate;
	}
    ...
    case 0xfe ... 0xff: /* Grp4/Grp5 */
        ...
        case 6: /* push */
            ...
            break;
        default:
	    printk("cannot emulate group 4/5\n");
            goto cannot_emulate;
        }
        break;
    }
...
// use unlocked (not SMP safe!) other
#ifdef NXDEBUG_DO_CMPXCHG
            if ( lock_prefix )
                rc = ops->cmpxchg(
 ...
 cannot_emulate:
    printk("pagefault handler: cannot emulate instruction stream:\n");
...
#define FAILED() do { printk("unhandled user trap at %s %s:%d\n", __FUNCTION__, __FILE__, __LINE__); } while (0)
...
int x86_emulate_insn_fetch_default(enum x86_segment seg,
  ...
  int ret = x86_emulate_do_read(offset, val, bytes);
...
int x86_emulate_do_write(unsigned long addr, unsigned long val, int bytes){
  switch(bytes){
  case 1:
    __asm__ __volatile__("movb %b1, %b0": "=m" (*(char *)addr): "q" (val));
    break;
  case 2:
    __asm__ __volatile__("movw %w1, %w0": "=m" (*(short *)addr): "r" (val));
    break;
  case 4:
    __asm__ __volatile__("movl %1, %0": "=m" (*(int *)addr): "r" (val));
    break;
  default:
    return X86EMUL_UNHANDLEABLE;
  };
...
int x86_emulate_do_read(unsigned long addr, unsigned long *val, int bytes){
  switch(bytes){
  case 1:
    __asm__ __volatile__("movzbl %b1, %0": "=q" (*val): "m" (*(char *)addr));
    break;
  case 2:
    __asm__ __volatile__("movzwl %w1, %0": "=r" (*val): "m" (*(short *)addr));
    break;
  case 4:
    __asm__ __volatile__("movl %1, %0": "=r" (*val): "m" (*(int *)addr));
    break;
  default:
    return X86EMUL_UNHANDLEABLE;
  };
