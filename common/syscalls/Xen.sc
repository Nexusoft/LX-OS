syscall Xen {
  ...
  decls __caller__ {
    ...
    int Xen_PreInit(void) {
      int rv = nexuscall0(SYS_RAW_Xen_PreInit_CMD);
  ...
  decls __callee__ {
	...
	printkx(PK_XEN, PK_WARN, "[xen] process has too many threads\n");
	return -SC_INVALID;
      }
      
      m = nexusthread_current_map();
      // Scan through memory map.
      // [0,NEXUS_VMM_START] => remove
	...
	if ((!(flags & PAGEFLAG_NULL)) // PTE valid (e.g. non-null parent PDE)
      ...
      printkx(PK_XEN, PK_INFO, "[xen] %d pages unmapped\n", num_unmapped);
      ...
      printkx(PK_XEN, PK_INFO, "[xen] switching process ring to CPL1\n");
      return 0;
    }
...
#define CHECK_XEN()						\
  ...
  interface int FreePages(unsigned int paddr, int numpgs) {
    ...
    CHECK_XEN();
    return freeKernelPages_Xen(nexusthread_current_ipd(), paddr, numpgs);
  }
