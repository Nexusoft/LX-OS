...
extern void * __ioremap(unsigned long offset, unsigned long size, unsigned long flags, int ioremap_protect);
...
{
  return __ioremap(offset, size, 0, 0);
}

/* map the region as not present */
static inline void * ioremap_protect (unsigned long offset, unsigned long size)
{
  return __ioremap(offset, size, 0, 1);
}
...
{
  return __ioremap(offset, size, _PAGE_PCD, 0);
}
...
#include <nexus/io_instrument.h>	// for inb, outb
#if 0
/*
