...
void pkcs_mgf1(unsigned char *output, int olen,
	       unsigned char *input,  int ilen)
{
  unsigned char *p = output;
  unsigned char h[20];
...
/*
 * Add the PKCS#1 v2.0 EME-OAEP padding and do a public RSA
 */
		       ...
		       unsigned char *pad,    int plen)
{
    int i;
    unsigned char *p = output;
    unsigned char *seed = p + 1;
    unsigned char *db = p + 1 + 20;

    if( olen != ctx->len || olen < ilen + 42 || olen < 42)
        return( ERR_RSA_BAD_INPUT_DATA );

    unsigned char *dbmask = galloc(olen - 21);
    unsigned char seedmask[20];
    ...
    *p++ = 0;
    ...
    sha1(pad, plen, p); // db: lhash
    p += 20;

    memset(p, 0, olen - ilen - 42); // db: PS
    p += olen - ilen - 42;

    *p++ = 0x01; // db: encrypt tag

    memcpy(p, input, ilen); // db: msg

    pkcs_mgf1(dbmask, olen - 21, seed, 20);
    for (i = 0; i < olen - 21; i++)
      db[i] ^= dbmask[i];

    pkcs_mgf1(seedmask, 20, db, olen - 21);
    for (i = 0; i < 20; i++)
      seed[i] ^= seedmask[i];
