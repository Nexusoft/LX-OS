...
void xendom_setupDomainPagetable(void) {
  ...
  void *new_ptable = getKernelPages(1);
...
static int xendom_remap_LDT(void) {
  ...
  for(i=0; i < FULL_LDT_PAGESIZE; i++) {
	...
	assert(0);
      ...
      if(fast_virtToPhys_nocheck(m, curr_page)) {
...
void xendom_KTS_free(BasicThread *t, KernelThreadState *kts) {
  ...
  if(kts->event_state != NULL) {
    EventChannelState_destroy(t, kts->event_state);
  }
  if(kts->per_domain_ptable != NULL) {
    nfree_page(kts->per_domain_ptable);
  }
  int i;
  for(i=0; i < FULL_GDT_PAGESIZE; i++) {
    Page *p = kts->xen_gdt.gdt_pages[i];
    if(p != NULL) {
      nfree_page(p);
    }
  }
...
static void vnet_down_tx_handler(IPD_ID source_ipd, Call_Handle call_handle, void *_ctx) {
  ...
  P_t(&hw->rx_queue_mutex);
  dlist_insert_tail(&hw->rx_queue, &pkt->link);
  V_t(&hw->rx_queue_mutex);
...
int xendom_setTrapTable(trap_info_t *new_entries) {
  ...
  KernelThreadState *kts = getMyKTS();
  if(kts->traps == NULL) {
    kts->traps = galloc(sizeof(NexusTrapInfo) * MAX_IDT_ENTRIES);
    int i;
    for(i=0; i < MAX_IDT_ENTRIES; i++) {
      NexusTrapInfo *ti = &kts->traps[i];
      ti->pending = 0;
      ti->cs = 0;
      ti->address = 0;
    }
...
void pass_to_cpl1_trap(int number, int have_error_code, 
  ...
  KernelThreadState *kts = getMyKTS();
  if(kts->traps == NULL) {
    printk("(%d): traps not set up for domain!\n", number);
    dump_regs_is(is);
    kill_current_domain();
  }
  if(1) {
    if(number == 6) {
      NexusTrapInfo *ti = &kts->traps[number];
      printk_red("(UD %x:%p => %x:%p (stack@%x:%p))", 
		 is->cs, (void *) is->eip,
		 ti->cs, (void *) ti->address,
		 kts->ss1, kts->esp1);
...
void do_xen_pfault(InterruptState *is) {
	...
	(PFEC_write_access | PFEC_page_present)) /* write*/ &&
      ...
      if(page != NULL && 
	 page->owner == nexusthread_current_ipd_id() && 
	 page->type == FT_PTABLE) {
...
void do_xen_systrap(InterruptState *is) {
  // printk_red("systrap(%x)", is->entry_vector);
...
TimerContext_new(UThread *ut, int is_periodic) {
  nexusthread_get((BasicThread *)ut);
...
static void TimerContext_destroy(TimerContext *ctx) {
  nexusthread_put((BasicThread *)ctx->target_thread);
  gfree(ctx);
}
...
static void start_virq_kbd(BasicThread *thread, int virq_num) {
  IPD *ipd = nexusthread_get_ipd(thread);
  NexusOpenDevice *nod = ipd_get_open_device(ipd, DEVICE_KEYBOARD, -1);
  if(nod != NULL) {
    kbd_set_xen_irq_thread(nod, thread);
  }
}
...
static void stop_virq_kbd(BasicThread *thread, int virq_num) {
  ...
  NexusOpenDevice *nod = ipd_get_open_device(ipd, DEVICE_KEYBOARD, -1);
  if(nod != NULL) {
    kbd_set_xen_irq_thread(nod, NULL);
  }
}
...
static void time_sync_fire(void *_ctx) {
  ...
  __u64 tsc_delta_in_update = 
    ((__u64)XEN_CLOCK_SYNC_PERIOD) * TSC_PER_TICK;
...
int xendom_fixup_seg(Selector sel, unsigned long offset) {
  ...
  if(desc == NULL || fast_virtToPhys_nocheck(m, (__u32) desc) == 0) {
