...
static int pcibios_lookup_irq(struct pci_dev *dev, int assign)
{
	...
	if (!pirq_table){
	  printk("pirq_table is null!!!\n");
	  return 0;
	}
	/* Find IRQ routing entry */
	pci_read_config_byte(dev, PCI_INTERRUPT_PIN, &pin);
	if (!pin) {
		printk(" -> no interrupt pin\n");
		return 0;
	}
	pin = pin - 1;
	...
	if (!info) {
		printk(" -> not found in routing table\n");
		return 0;
	}
	...
	printkx(PK_PCI, PK_DEBUG, " -> PIRQ %02x, mask %04x, excl %04x", pirq, mask, pirq_table->exclusive_irqs);
	...
	newirq = dev->irq;
	if (!newirq && assign) {
		printk("sorry, can't assign irq line dynamically\n");
		nexuspanic();
#if 0
		int i;
		for (i = 0; i < 16; i++) {
	...
	printkx(PK_PCI, PK_DEBUG, " -> newirq=%d", newirq);

	/* Check if it is hardcoded */
	if ((pirq & 0xf0) == 0xf0) {
		irq = pirq & 0xf;
		printkx(PK_PCI, PK_DEBUG, " -> hardcoded IRQ %d\n", irq);
		msg = "Hardcoded";
	} else if (r->get && (irq = r->get(pirq_router_dev, dev, pirq))) {
		printkx(PK_PCI, PK_DEBUG, " -> got IRQ %d\n", irq);
		msg = "Found";
	} else if (newirq && r->set && (dev->class >> 8) != PCI_CLASS_DISPLAY_VGA) {
		printkx(PK_PCI, PK_DEBUG, " -> assigning IRQ %d", newirq);
		if (r->set(pirq_router_dev, dev, pirq, newirq)) {
			eisa_set_level_irq(newirq);
			printkx(PK_PCI, PK_DEBUG, " ... OK\n");
			msg = "Assigned";
			irq = newirq;
		}
	}
	...
	printkx(PK_PCI, PK_DEBUG, "PCI: %s IRQ %d for device %s\n", msg, irq, dev->slot_name);
...
void __init pcibios_irq_init(void)
{
	...
	if (pirq_table) {
		...
		//printk("io_apic_assign = %x\n", io_apic_assign_pci_irqs);
		//nexuspanic();
		if (0 && io_apic_assign_pci_irqs)
			pirq_table = NULL;
	}
}
...
void __init pcibios_fixup_irqs(void)
{
	 ...
	 */
		...
		if (pin && !dev->irq){
		  //printk
		  //nexuslog("DAN: looking up irq 0x%p %d (%d)\n", dev, 0, __LINE__);
...
void pcibios_enable_irq(struct pci_dev *dev)
{
	...
	//	printk("DAN: pcibios_enable_irq %s\n", dev->name);
	pci_read_config_byte(dev, PCI_INTERRUPT_PIN, &pin);
