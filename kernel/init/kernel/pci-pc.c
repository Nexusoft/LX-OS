...
extern int pci_conf1_read_config_byte(struct pci_dev *dev, int where, u8 *value)
{
	int result; 
	u32 data;
	//printk("DAN: pci_conf1_read_config_byte\n");
...
extern int pci_conf1_read_config_word(struct pci_dev *dev, int where, u16 *value)
{
	int result; 
	u32 data;
//	printk("DAN: pci_conf1_read_config_word\n");
//	printk("dev=0x%p ", dev);
//	printk("dev->bus=0x%p ", dev->bus);
//	printk("dev->bus->number=%d ", dev->bus->number);
//	printk("PCI_SLOT(%d)=0x%x ", dev->devfn, PCI_SLOT(dev->devfn));
//	printk("PCI_FUNC(%d)=0x%x ", dev->devfn, PCI_FUNC(dev->devfn));
//	printk("where=%d ", where);
//	printk("value=0x%p\n", value);
...
//extern struct pci_ops *ops_ptr = &pci_direct_conf1;
					...
					unsigned short index, unsigned char *bus, unsigned char *device_fn)
{
	unsigned short bx;
	unsigned short ret;
	//	printk("DAN: pcibios_find_device\n");
	__asm__("lcall *(%%edi); cld\n\t"
		"jc 1f\n\t"
		"xor %%ah, %%ah\n"
		"1:"
...
static int pci_bios_read_config_byte(struct pci_dev *dev, int where, u8 *value)
{
	...
	if (!value) 
		BUG();
	//printk("DAN: pci_bios_read_config_byte\n");
...
static int pci_bios_read_config_word(struct pci_dev *dev, int where, u16 *value)
{
	int result; 
	u32 data;
	//printk("DAN: pci_bios_read_config_word\n");
	if (!value) 
		BUG();
...
{
	printk("pcibios_get_irq_routing_table\n");
	nexuspanic();
	return NULL;
#if 0
	...
	if (ret & 0xff00)
		...
		rt = galloc(sizeof(struct irq_routing_table) + opt.size);
		if (rt) {
...
void __devinit pcibios_config_init(void)
{
	 ...
	 */
  //  printk("DAN: pcibios_config_init\n");
#ifdef CONFIG_PCI_DIRECT
...
{
  //  printk("DAN: pcibios_assign_all_busses\n");
...
{
	int err;
	//printk("DAN: pcibios_enable_device\n");
