...
#endif
  struct BasicThread *fxstate_owner;
...
struct BasicThread;
struct BasicThread *curt;

static inline void
intcontext_enter(void) 
{
  if (likely(curt != NULL))
...
static inline void
intcontext_exit(void)
{
  if (likely(curt != NULL))
  	cpu_state[current_cpu()].in_interrupt = 0;
}

/** Return 1 if in interrupt context, 0 otherwise */
static inline int
intcontext_check(void)
{
	return cpu_state[current_cpu()].in_interrupt;
}

//// debugging: stack trace

void dump_user_stack(BasicThread *thread, unsigned int ebp);
void dump_stack_current(struct InterruptState *is);	
