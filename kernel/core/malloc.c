...
static void __debug_malloc_failure(const char *file, int line)
{
	int i;

	printk("malloc failure at %s:%d\n", file, line);
}

static void __debug_nexuspanic_malloc(void)
{
	__debug_malloc_failure("malloc.c:abort()", 0);
}

static inline int malloc_getpagesize(void)
{
	return PAGESIZE;
}

static inline unsigned long __debug_malloc_self(void)
{
#ifdef CHECK_INTERRUPT_CONTEXT
	if (cpu_in_interrupt()) {
		printk("WARN: alloc in probable interrupt\n");
		return 0;	// not used by nexusthread ids
	}
	else 
#endif
	{
...
#define MMAP(s)		kernel_mmap(s)
#define DIRECT_MMAP(s)	MMAP(s)
#define MREMAP(a,b,c,d)	kernel_mremap()
#define MUNMAP		kernel_munmap
#define MORECORE gmorecore
...
#define ABORT 			__debug_nexuspanic_malloc

#ifndef NDEBUG
#define DEBUG 1			/* XXX: remove at some point */
#define FOOTERS 1		/* be paranoid */
#define MALLOC_FAILURE_ACTION	__debug_malloc_failure(__FILE__, __LINE__);
#else
...
#ifdef __NEXUSKERNEL__
#define CURRENT_THREAD        __debug_malloc_self()
#else
...
#ifdef __NEXUSKERNEL__	/* gcc defines __linux__ even on nexus */
      printk("NXDEBUG: busy waiting in malloc\n");
      //nexusthread_yield();
      return 0;
#else
      sched_yield();
...
#else
        magic = (size_t)(nexustime ^ (size_t)0x55555555U);
#endif
...
static void internal_malloc_stats(mstate m) {
  ...
  if (!PREACTION(m)) {
    ...
    nxcompat_fprintf(stderr, "max system bytes = %10lu\n", (unsigned long)(maxfp));
    nxcompat_fprintf(stderr, "system bytes     = %10lu\n", (unsigned long)(fp));
    nxcompat_fprintf(stderr, "in use bytes     = %10lu\n", (unsigned long)(used));
