...
/* Disabled because the kfree symbol is not declared anywhere.
   I don't think this code is exercised. Fail loudly if it is.*/
#ifdef __NEXUSKERNEL__
		DBGC((KERN_ERR "Trying to call non-implemented function kfree\n"));
		asm ("hlt");
#else
		kfree(tmp);
#endif
...
/* disabled because x86 doesn't need fixing up
   (it seems: this function is not implemented on that arch */
#ifdef __NEXUSKERNEL__
	DBGC((KERN_ERR "Trying to call non-implemented function pcibios_fixup_...\n"));
	asm ("hlt");
#else
