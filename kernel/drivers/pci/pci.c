/*
 *	$Id: pci.c 2921 2007-06-19 20:24:58Z ashieh $
 *
...
pci_match_device(const struct pci_device_id *ids, const struct pci_dev *dev)
{
	while (ids->vendor || ids->subvendor || ids->class_mask) {
		if ((ids->vendor == PCI_ANY_ID || ids->vendor == dev->vendor) &&
			...
			printkx(PK_INFO, PK_PCI, "[pci] driver found for device %x.%x\n",
	...
	printkx(PK_DEBUG, PK_PCI, "[pci] driver does not match device %x.%x\n", 
		dev->vendor, dev->device);
	return NULL;
}
...
pci_set_master(struct pci_dev *dev)
{
	...
	if (! (cmd & PCI_COMMAND_MASTER)) {
		printkx(PK_PCI, PK_DEBUG, "PCI: Enabling bus mastering for device %s\n", dev->slot_name);
...
pci_set_mwi(struct pci_dev *dev)
{
	...
	if (! (cmd & PCI_COMMAND_INVALIDATE)) {
		printkx(PK_PCI, PK_DEBUG, "PCI: Enabling Mem-Wr-Inval for device %s\n", dev->slot_name);
