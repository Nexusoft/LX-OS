...
extern void vortex_up(struct net_device *dev);
static void vortex_down(struct net_device *dev);
extern int vortex_open(struct net_device *dev);
...
extern int boomerang_start_xmit(struct sk_buff *skb, struct net_device *dev);
static int vortex_rx(struct net_device *dev);
static int boomerang_rx(struct net_device *dev);
static void boomerang_refill_rx(void *arg);
static void vortex_interrupt(int irq, void *dev_id, struct pt_regs *regs);
extern void boomerang_interrupt(int irq, void *dev_id, struct pt_regs *regs);
static int vortex_close(struct net_device *dev);
static void dump_tx_ring(struct net_device *dev);
static void update_stats(long ioaddr, struct net_device *dev);
extern struct net_device_stats *vortex_get_stats(struct net_device *dev);
extern void set_rx_mode(struct net_device *dev);
...
static int __init vortex_eisa_init (void)
{
	...
	for (ioaddr = 0x1000; ioaddr < 0x9000; ioaddr += 0x1000) {
		...
		//DAN: vortex_probe1 doesn't return an int anymore!
		rc = 1;
...
void dump_pdev_short(struct pci_dev *pdev){
	int i;
...
static int vortex_init_one (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	struct net_device *rc;
	int ret=0;
...
//	printk("DAN: vortex_probe1, 0x%lx 0x%x 0x%x 0x%x\n", ioaddr, irq, chip_idx, card_idx);
...
//	printk("DAN: about to pci_alloc_consistent\n");
...
//	printk("DAN: done pci_alloc_consistent\n");
	...
	EL3WINDOW(0);
	{
		...
		//printk("DAN: eprom 0x%x 0x%lx 0x%x 0x%x\n", base, ioaddr, Wn0EepromCmd, Wn0EepromData);
		for (i = 0; i < 0x40; i++) {
			int timer;
...
//			(dev->features & NETIF_F_IP_CSUM) ? "en":"dis");
	...
	retval = nexus_register_netdev(dev, "3c59x", boomerang_interrupt);
	if (retval == 0)
		return dev;
...
vortex_up(struct net_device *dev)
{
	...
	if (dev->if_port == XCVR_MII || dev->if_port == XCVR_NWAY) {
		...
		if (vortex_debug > 1) {
			...
			int advertising = mdio_read(dev, vp->phys[0], 4);
			printk(KERN_DEBUG "%s: Advertising %04x\n",
				   dev->name, advertising);
		}
...
static int
vortex_start_xmit_data(void *data, int datalen, struct net_device *dev)
{
	printk("DAN: %s %d\n", __FILE__, __LINE__);
	nexuspanic();
	return -1;
...
boomerang_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (!skb_shinfo(skb)->nr_frags) {
		...
		for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
			...
			printk("DAN: transmit needs page_address\n");
			nexuspanic();
#if 0
			// nb: shouldn't the next be i-1?
	...
	if (vp->cur_tx - vp->dirty_tx > TX_RING_SIZE - 1) {
		printk("3c59x: no space on ring. TODO: quiesce device\n");
	}
	outw(DownUnstall, ioaddr + EL3_CMD);
	dev->trans_start = nexustime;
	//printk("Boomerang: started transmission\n");
	return 0;
}
...
void boomerang_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
...
boomerang_rx(struct net_device *dev)
{
	...
	while ((rx_status = le32_to_cpu(vp->rx_ring[entry].status)) & RxDComplete) {
		...
		if (rx_status & RxDError) { /* Error, update stats. */
			...
			// printk("(size=%d stat=%4.4x)\n", pkt_len, rx_status, entry);
			if (0) {
				/* copy and allocate (in interrupt context: BUG) */
				skb = skb_allocate(pkt_len + 2);

				skb->dev = dev;
				skb_reserve(skb, 2);	/* Align IP on 16 byte boundaries */
				pci_dma_sync_single(vp->pdev, dma, PKT_BUF_SZ, PCI_DMA_FROMDEVICE);
				memcpy(skb_put(skb, pkt_len), vp->rx_skbuff[entry]->tail, pkt_len);
				vp->rx_copy++;
			} else {
				/* Pass up the skbuff already on the Rx ring. */
				/* XXX: verify that the entry is given back at some point */
			...
			task_sched(boomerang_refill_rx, dev);
			
			dev->last_rx = nexustime;
			vp->stats.rx_packets++;
		}
...
/*
 * Refill the Rx ring. Simple function that can be scheduled in a
 * background worker thread. 
 *
 * This code used to be part of boomerang_rx
 */
static void boomerang_refill_rx(void *arg)
{
	struct net_device *dev;
...
/*  Update statistics.
	...
	*/
static void 
update_stats(long ioaddr, struct net_device *dev)
{
	struct vortex_private *vp = (struct vortex_private *)dev->priv;
...
static int 
vortex_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	struct vortex_private *vp = (struct vortex_private *)dev->priv;
	long ioaddr = dev->base_addr;
	...
	int retval;
	printk("vortex_ioctl 0x%x\n", cmd);
	printk("ETHTOOL 0x%x MIIPHY 0x%x DEVPRIV 0x%x MIIREG 0x%x\n", SIOCETHTOOL, SIOCGMIIPHY, SIOCDEVPRIVATE, SIOCSMIIREG);
...
mdio_read(struct net_device *dev, int phy_id, int location)
{
	...
	long mdio_addr = ioaddr + Wn4_PhysicalMgmt;
	//	printk("mdio_read\n");
	//DAN: taking out the spin lock because the unlock at the 
	// bottom of this function crashes us
	//spin_lock_bh(&vp->mdio_lock);
	//printk("spinlock is gone\n");
...
static void 
acpi_set_WOL(struct net_device *dev)
{
	struct vortex_private *vp = (struct vortex_private *)dev->priv;
	long ioaddr = dev->base_addr;
	printk("acpi_set_WOL\n");
...
static void 
__devexit vortex_remove_one (struct pci_dev *pdev)
{
	printk("DAN: %s:%d\n", __FILE__, __LINE__);
	nexuspanic();
}
...
//DAN: static int __init vortex_init (void)
int __init 
vortex_init (void)
{
	int pci_rc, eisa_rc;
