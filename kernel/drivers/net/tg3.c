...
#ifdef NEXUS_UDRIVER
#define LINUX_VERSION_CODE 0x020422
#else
...
#ifndef NONEXUS
// userdriver implementation of boilerplate
#ifdef NEXUS_UDRIVER
void assert(int val)    { if (!val) { printk("assertion failed at %s.%d\n", __FUNCTION__, __LINE__); abort(); } }
void msleep(int msecs)  { usleep (msecs * 1000); }
int pci_find_capability (struct pci_dev *dev, int cap) { return 0; }
#define nexuspanic(X)	assert(0)
#define V(X)		V_nexus(X)
#endif
#endif
...
#ifndef NONEXUS
#ifdef NEXUS_UDRIVER
#define SEMA_INIT_KILLABLE SEMA_INIT
#endif

static Sema tg3_nexus_sema = SEMA_INIT_KILLABLE;
static Sema tg3_nexus_sema_start = SEMA_INIT_KILLABLE;
static Sema tg3_nexus_sema_reset = SEMA_INIT_KILLABLE;

static int tg3_background_nexus(void *_dev);
static int tg3_background_nexus_reset(void *_tp);
static void tg3_netif_rx_schedule_nexus(void);
static void tg3_schedule_work_nexus(void);
...
// Nexus requires ethernet frames to be page-aligned
#ifdef NONEXUS
	...
	skb->data = skb->head;
	skb->len += ETH_HLEN;
	mapping = nexuscompat_pci_map_single(tp->pdev, skb->data, skb_size,
				 PCI_DMA_FROMDEVICE);
#endif
...
// Nexus swaps full pages. Skip per-byte optimizations
#ifdef NONEXUS
...
#ifdef TG3_NAPI
#ifndef NONEXUS
#error "Nexus does not support NAPI"
#endif
...
{
#if defined NONEXUS || defined __NEXUSKERNEL__
	spin_lock_bh(&tp->lock);
	if (irq_sync)
		tg3_irq_quiesce(tp);
#endif
...
#endif
{
...
#else
	{
		if (tp->timer)
			deregister_alarm(tp->timer);
		tp->timer = register_alarm(nexustime + 2, (void *) tg3_timer, tp);
	}
#endif
#endif
				       ...
				       u32 *start, u32 base_flags, u32 mss)
{
	...
	if (!new_skb) {
		...
		new_addr = pci_map_single(tp->pdev, new_skb->data, new_skb->len,
					  PCI_DMA_TODEVICE);
#else
		new_addr = nexuscompat_pci_map_single(tp->pdev, new_skb->data, new_skb->len,
					  PCI_DMA_TODEVICE);
#endif
...
#ifndef NONEXUS
#error "Nexus does not support TSO"
#endif
	mss = 0;
...
#else
	mapping = nexuscompat_pci_map_single(tp->pdev, skb->data, len, PCI_DMA_TODEVICE);
#endif
	if (tg3_pci_dma_mapping_error(tp->pdev, mapping)) {
		dev_kfree_skb(skb);
		goto out_unlock;
...
#else
	printk("[tg3] offloaded Tx pseudoheader calculation to dev\n");
#endif
...
#else
#ifdef __NEXUSKERNEL__
	tp->timer = register_alarm(tp->timer_offset, (void *) tg3_timer, tp);
#else
	// noop: started a recurrent timer
	; // semicolon required for label at end of compound statement
#endif
#endif
}
...
#else
		nexuspanic();	// MSI not supported in Nexus
#endif
	} else {
		fn = tg3_interrupt;
		if (tp->tg3_flags & TG3_FLAG_TAGGED_STATUS)
...
static int tg3_open(struct net_device *dev)
{
	...
	tp->timer = register_alarm(tp->timer_offset, (void *) tg3_timer, tp);
#else
	nxcall_every_second(tg3_timer, tp);
#endif
#endif
	tp->tg3_flags |= TG3_FLAG_INIT_COMPLETE;
	tg3_enable_ints(tp);
...
#else
	map = nexuscompat_pci_map_single(tp->pdev, skb->data, tx_len, PCI_DMA_TODEVICE);
#endif
...
#ifdef NEXUS_UDRIVER
	dev = alloc_etherdev("tg3", sizeof(*tp));
#else
...
#ifndef NONEXUS
#ifdef __NEXUSKERNEL__
	nexusthread_fork(tg3_background_nexus, dev);
	nexusthread_fork(tg3_background_nexus_reset, tp);
#else
	{
		int t;
		pthread_create(&t, NULL, tg3_background_nexus, dev);
		pthread_create(&t, NULL, tg3_background_nexus_reset, tp);
	}
#endif
	P(&tg3_nexus_sema_start);
	P(&tg3_nexus_sema_start);
#endif
...
#else
	err = nexus_register_netdev(dev, "tg3", tg3_interrupt, &tp->napi[0]);
#endif
	if (err) {
...
#else
int __init tg3_init(void)
#endif
{
#ifdef NEXUS_UDRIVER
	/* TG3 communicates with the PHY layer through a word in shared memory.
	   The userspace driver can successfully grant access to necessary ioports
	   and map driver memory into userspace, but this communication protocol
	   fails: the driver writes data to the word and then polls for the device
	   to return a non-busy signal together with data. Currently, the device
	   consistently returns 0xffffffff. This behavior is observed both when
	   using the memory read/write traps in Device.sc and when memory is mapped
	   read/write (and thus accesses do not trap).
	 */
	printk("WARNING: userspace driver is BROKEN. See %s.%d for detail\n",
	       __FUNCTION__, __LINE__);
	return pci_register_driver(&tg3_driver, 0);
#else
...
/** Worker thread for reset operations */
static int tg3_background_nexus_reset(void * _tp)
{
	struct tg3 *tp = _tp;

	V(&tg3_nexus_sema_start);
	while (1) {
		P(&tg3_nexus_sema_reset);
		tg3_reset_task(tp);
	}
...
static void tg3_schedule_work_nexus(void)
{
	V(&tg3_nexus_sema_reset);
}

/** Worker thread scheduled from the interrupt handler when there is work */
static int tg3_background_nexus(void *_dev)
{
	...
	V(&tg3_nexus_sema_start);
#else
	V_nexus(&tg3_nexus_sema_start);
#endif
	while (1) {
		P(&tg3_nexus_sema);

		budget = dev->weight;
		while (tg3_poll(dev, &budget)) {
#ifdef __NEXUSKERNEL__
			nexusthread_yield();
#else
			Thread_Yield();
...
/** Wake up the background handler thread */
static void tg3_netif_rx_schedule_nexus(void)
{
	V(&tg3_nexus_sema);
}
