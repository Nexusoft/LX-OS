...
//#define PING() printk_current("PING(%s:%d; %s)\n", __FILE__, __LINE__, __FUNCTION__)
#define PING()
...
// OLIVER: Moved from compat definitions to override the one in tg3.h
#define WARN_ON(condition) do { if(unlikely(condition)) { printk("Warning at %s:%d\n", __FILE__, __LINE__); } } while(0)

#include "tg3.h"

Sema *tg3_poll_start_sema;
Sema *tg3_poll_sema;

// ALAN: 2.6 & 2.4.31 compatibility definitions
static inline void netif_rx_schedule_nexus(struct net_device *dev);

#define ASSERTNOTCALLED() do { printk("%s SHOULD NOT BE CALLED\n", __FUNCTION__); dump_stack(); nexuspanic(); } while(0)
#define PRINT_NOTIMPLEMENTED() printk("%s NOT IMPLEMENTED\n", __FUNCTION__)

// OLIVER: This found its way into tg3.h
...
//Oliver: ik.  Hopefully this won't break anything.
static inline void msleep(int msec){
  nexusthread_sleep(msec);  
}

static inline int msleep_interruptible(int msec){
  ASSERTNOTCALLED();
  return 0;
}
...
u32 ethtool_op_get_tx_csum(struct net_device *);
u32 ethtool_op_get_sg(struct net_device *);
int ethtool_op_set_sg(struct net_device *, u32);
...
//	if(dummyfoobarbaz > 0){
//	 printf("work_status: %p %x %d %d (%x %x)\n", sblk, tp->tg3_flags, 
//	   sblk->idx[0].tx_consumer, sblk->idx[0].rx_producer,
...
{
	tp->dev->trans_start = nexustime;	/* prevent tx timeout */
#if 0 // ALAN
	...
	if (GET_ASIC_REV(tp->pci_chip_rev_id) == ASIC_REV_5906) {
		...
		for (i = 0; i < 200; i++) {
			...
			if (val == ~NIC_SRAM_FIRMWARE_MBOX_MAGIC1)
				break;
//Oliver: msleep is used in the original tg3.  Must ask Alan how this should be handled..
...
static int tg3_poll(struct net_device *netdev, int *budget)
{
	...
	if (done) {
		...
		//Oliver: netif_rx_complete(netdev);
		tg3_restart_ints(tp);
...
#endif
{
	...
	if (likely(!tg3_irq_sync(tp)))
		netif_rx_schedule_nexus(dev);		/* schedule NAPI poll */
...
static void tg3_timer(void *__opaque);
...
//  printk("R");
	...
	if ((sblk->status & SD_STATUS_UPDATED) ||
		...
		if (likely(tg3_has_work(tp))) {
			 ...
			 */
			tg3_timer(tp); // XXX: hack to force timer process to run (ashieh)
...
static int tg3_restart_hw(struct tg3 *tp, int reset_phy)
{
	...
	if (err) {
		...
		tg3_full_unlock(tp);
//Oliver:		del_timer_sync(&tp->timer);
    deregister_alarm(&tp->timer);
		tp->irq_sync = 0;
//OLIVER: Let's hope this doesn't break anything...
//		netif_poll_enable(tp->dev);
//		dev_close(tp->dev);
		tg3_full_lock(tp, 0);
	}
...
#if 0 // ALAN: Timers may not actually be needed
	if (restart_timer)
		mod_timer(&tp->timer, nexustime + 1);
#endif
...
static int tg3_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    printk("DAN: we never have fragments\n");
    nexuspanic();
#if 0
		unsigned int i, last;
...
static int tg3_start_xmit_dma_bug(struct sk_buff *skb, struct net_device *dev)
{
	...
	// hack to deal with broken vSwitch. XXX: still needed?
	if (1) {
		uint64_t blank = 0;

		if (!memcmp(skb->data + 6, dev->dev_addr, 6) ||
		    !memcmp(skb->data + 6, &blank, 6)) {
			printk_red("bad source for packet\n");
			dev_kfree_skb(skb);
			return NETDEV_TX_OK;
		}
	}
	...
	/* Now loop through additional data fragments, and queue them. */
	if (skb_shinfo(skb)->nr_frags > 0) {
    printk("Oliver: we never have fragments... do we?\n");
    nexuspanic();
#if 0
		unsigned int i, last;
...
{
	tp->rx_std_buffers = galloc((sizeof(struct ring_info) *
				      (TG3_RX_RING_SIZE +
				       TG3_RX_JUMBO_RING_SIZE)) +
...
restart_timer:
	...
	//Oliver: Nexus alarms are a touch different
	//add_timer(&tp->timer);
	//tp->timer.expires = nexustime + tp->timer_offset;
  if(tp->timer.fired == 1){
    tp->timer.alarmtime = nexustime + tp->timer_offset;
    register_alarm_norelease(&tp->timer);
  }
}
...
static int tg3_request_irq(struct tg3 *tp)
{
  ...
  if(1){	//Oliver: Trying to figure out why the hell the card is spamming interrupts!
    if (tp->tg3_flags2 & TG3_FLG2_USING_MSI) {
      fn = tg3_msi;
      if (tp->tg3_flags2 & TG3_FLG2_1SHOT_MSI)
        fn = tg3_msi_1shot;
...
static int tg3_open(struct net_device *dev)
{
    ...
    //Oliver: Nexus timer semantics are a touch different.
		//init_timer(&tp->timer);
		//tp->timer.alarmtime = nexustime + tp->timer_offset;
		//tp->timer.data = (unsigned long) tp;
		//tp->timer.function = tg3_timer;
		memset(&tp->timer, 0, sizeof(Alarm));
		tp->timer.alarmtime = nexustime + tp->timer_offset;
		tp->timer.arg = (void *) tp;
		tp->timer.func = tg3_timer;
	}
...
#if 0
	// ASHIEH: Unconditionally set rx mode
	if (!netif_running(dev))
		return;
#endif
	tg3_full_lock(tp, 0);
...
static struct ethtool_ops __attribute__((unused)) tg3_ethtool_ops = {
				  ...
				  const struct pci_device_id *ent)
{
	...
	dev->poll = tg3_poll;
	// ALAN: dev->ethtool_ops = &tg3_ethtool_ops;
	dev->weight = 64;
	...
	tg3_poll_start_sema = sema_new();
	tg3_poll_sema = sema_new();
	int nexus_tg3_pollhelper(void *arg);
	nexusthread_fork(nexus_tg3_pollhelper, dev);
	P(tg3_poll_start_sema);
	sema_destroy(tg3_poll_start_sema);
	...
	err = nexus_register_netdev(dev, "tg3", tg3_interrupt);
	if (err) {
...
// ALAN: Definitions of ethtool stubs
...
int ethtool_op_set_sg(struct net_device *dev, u32 sg) {
	PRINT_NOTIMPLEMENTED();
	return 0;
}
...
// NAPI polling implementation of rx 
int nexus_tg3_pollhelper(void *arg) {
	struct net_device *netdev = arg;
	V(tg3_poll_start_sema);
		for (;;) {
		P(tg3_poll_sema);
		int budget = netdev->weight;
		// if(netdev->quota == 0) printk("quota = 0\n");
		while(tg3_poll(netdev, &budget) == 1) {
			nexusthread_yield();
...
static inline void netif_rx_schedule_nexus(struct net_device *dev)
{
  ...
  //printk("DAN: netif_rx_schedule_nexus\n");

  //if (netif_rx_schedule_prep(dev)){
  //local_irq_save(flags);
  
  //if (dev->quota < 0)
  //dev->quota += dev->weight;nnn
  // else
  ...
  //}
    ...
    //printk("DAN: netif_rx_schedule_nexus done\n");
}
