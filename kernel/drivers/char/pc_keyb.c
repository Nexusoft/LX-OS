...
static inline void handle_mouse_event(unsigned char scancode)
{
	...
	// Always hand up to Nexus layer if this isn't an ack of a low-level send
	mouse_handleScancode(scancode);

	if(scancode == AUX_RECONNECT2 && prev_code == AUX_RECONNECT1
		&& aux_reconnect) {
...
static inline void handle_keyboard_event(unsigned char scancode)
{
	...
	//printk("DAN: scheduling tasklet (pc_keyb.c handle_keyboard_event)\n");
	//tasklet_schedule(&keyboard_tasklet);
	//DAN: everything seems to be scheduling a task or tasklet
	//DAN: handle_scancode is in keyboard.c
	//DAN: what is keyboard_tasklet??? kbd_bh in keyboard.c
}	

void alt_tab_event(int shift_on) {
  assert(check_intr() == 0);
  //KBDFunc *event;
  //printk_current("!");
  /* XXX this is a bug: we should not be mallocing on the top half! */
  //event = (KBDFunc *)galloc(sizeof(KBDFunc));
  if(!shift_on) {
    task_sched(focus_next, NULL);
    //event->func = focus_next;
  } else {
    task_sched(focus_prev, NULL);
    //event->func = focus_prev;
  }
  //queue_append(kbdeventq, event);
  //V(kbdevent);
 ...
 * spinlock.
 */
unsigned char lastscancode;
int alt_on = 0, alt_tab_on = 0;

extern int in_panic;
extern char in_panic_scancode;
...
static unsigned char handle_kbd_event(void)
{
  ...
  int dbg = 0;
	...
	while ((--work > 0) && (status & KBD_STAT_OBF)) {
		...
		if (in_panic) {
		  // let the panic code handle the keypress in its own way
		  in_panic_scancode = scancode;
		  return; 
		}
		...
		if((alt_on == 1) && (scancode == 15)){
		  alt_tab_on = 1;
		  //printk("!alt-tab on!\n");
		}
		if((alt_tab_on == 1) && ((scancode == 143)||(scancode == 184))){
		  alt_tab_on = 0;
		  //printk("!alt-tab event!\n");
		  alt_tab_event(shift_on);
		}

		if ((lastscancode == 0xe1) && (scancode == 0x1d)) {
		  trace_pause();
		}
...
void nexus_keyboard_interrupt(int irq, NexusDevice *nd) {
  
  extern void nexusdumprunqueue(void);

  assert(check_intr() == 0); /* called from interrupt context */
  keyboard_interrupt(irq, NULL, NULL);
  assert(check_intr() == 0); /* called from interrupt context */
}
...
static int mousedev_write(NexusDevice *nd, const char *data, int count) {
	// based on write_aux()
	ssize_t retval = 0;
...
static struct device_mouse_ops mouse_ops = {
	.write = mousedev_write,

	.protocol = MPROT_DEFAULT,
	.head = 0,
	.tail = 0,
	.curr_packet = 0,
};

int nexus_mouse_interrupt(int irq, NexusDevice *nd) {
	// Mouse handling is done by keyboard driver
	keyboard_interrupt(irq, NULL, NULL);
	return 0;
}
...
extern NexusDevice *nexus_mouse_device;

static void set_initial_psaux_protocol(void) {
	mouse_parse_disable();
	int dev_id;
	do {
		dev_id = nd_GetDeviceID(nexus_mouse_device);
		// printk_red("got devid %d ", dev_id);
	} while(dev_id < 0);
	nd_set_protocol(nexus_mouse_device, MPROT_DEFAULT);
	// nd_set_protocol(nexus_mouse_device, MPROT_EXPPS2);
	mouse_parse_enable();
}

int __init psaux_init(void)
{
	if(nexus_mouse_device != NULL) {
		printk_red("psaux already initialized, resetting mouse protocol\n");
		set_initial_psaux_protocol();
		return 0;
	}
	...
	nexus_mouse_device = 
		nexus_register_device(DEVICE_MOUSE, "mouse", 
				      AUX_IRQ, &mouse_ops,
				      nexus_mouse_interrupt,
				      mouse_focus,
				      DRIVER_KERNEL);
...
void nexus_leds(unsigned int leds){
  static char led;
  led = ((leds & 0x2) << 1) | ((leds & 0x4) >> 1) | (leds & 0x1);
  while (kbd_read_status() & KBD_STAT_IBF) mdelay(1); 
  kbd_write_output(KBD_CMD_SET_LEDS);
  mdelay(1); 
  while (kbd_read_status() & KBD_STAT_IBF) mdelay(1); 
