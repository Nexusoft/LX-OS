/* ATTENTION NEXUS: Leave this file (and the rest of them related to
   the audio driver) in the kernel, as we may use them for a
...
static inline unsigned i810_get_dma_addr(struct i810_state *state, int rec)
{
	...
	/* printk_djwill("civ=0x%x offset=%d fragsize=%d dmasize=%d, bytes=%d\n", 
		      civ, offset, dmabuf->fragsize, dmabuf->dmasize, bytes); */
...
static void i810_update_ptr(struct i810_state *state)
{
	...
	if (dmabuf->enable == DAC_RUNNING) {
		...
		if ((dmabuf->count < (int)(dmabuf->dmasize-dmabuf->userfragsize)) && (curr_audio_buf->waiting > 0)){
#ifdef DEBUG_LOCKING
			printk("waking\n");
#endif
...
int i810_interrupt_nexus(int irq, NexusDevice *nd) {
	i810_interrupt(irq, mycard, NULL);
	return 0;
}
...
static int i810_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct ac97_codec *codec = state->card->ac97_codec[0];
#endif
	struct i810_state *state = mycard->states[0]; // file->private_data;
	struct i810_channel *c = NULL;
	struct dmabuf *dmabuf = &state->dmabuf;
	unsigned long flags;
	audio_buf_info abinfo;
...
int i810_ioctl_nexus(unsigned int cmd, unsigned long arg, Map *m){
	int dbg_ioctl = 0;
	struct i810_state *state = mycard->states[0]; //(struct i810_state *)file;
	struct i810_channel *c = NULL;
	struct dmabuf *dmabuf = &state->dmabuf;
	...
	case OSS_GETVERSION:
		...
		return poke_user(m, arg, (void *)&soundversion, sizeof(int));
	...
	case SNDCTL_DSP_SPEED: /* set smaple rate */
		...
		//val = arg;
		if (peek_user(m, arg, (void *)&val, sizeof(int)))
		   return -EFAULT;
		...
		return poke_user(m, arg, (void *)&dmabuf->rate, sizeof(int));
		//return 0;
	...
	case SNDCTL_DSP_STEREO: /* set stereo or mono channel */
		...
		return poke_user(m, arg, (void *)&one, sizeof(int));
	...
	case SNDCTL_DSP_GETBLKSIZE:
		...
		return poke_user(m, arg, (void *)&dmabuf->userfragsize, sizeof(int));

	case SNDCTL_DSP_GETFMTS: /* Returns a mask of supported sample format*/
		if(dbg_ioctl){
			printk("SNDCTL_DSP_GETFMTS\n");
		}
		return poke_user(m, arg, (void *)&afmt_s16_le, sizeof(int));
	...
	case SNDCTL_DSP_CHANNELS:
		...
		if (val > 0) {
			...
			return poke_user(m, arg, (void *)&state->card->channels, sizeof(int));
		}

		/* ICH and ICH0 only support 2 channels */
		if ( state->card->pci_id == PCI_DEVICE_ID_INTEL_82801 
		     || state->card->pci_id == PCI_DEVICE_ID_INTEL_82901) 
			return poke_user(m, arg, (void *)&two, sizeof(int));
		...
		return poke_user(m, arg, (void *)&val, sizeof(int));
...
#endif
		//return copy_to_user((void *)arg, &abinfo, sizeof(abinfo)) ? -EFAULT : 0;
		poke_user(m, arg, (void *)&abinfo, sizeof(abinfo));
		return 0;
	case SNDCTL_DSP_GETOPTR:
		if (!dmabuf->ready && (val = prog_dmabuf(state, 0)) != 0)
			return val;
		...
		printk("SNDCTL_DSP_GETOPTR %d, %d, %d, %d\n", cinfo.bytes,
		       cinfo.blocks, cinfo.ptr, dmabuf->count);
#endif
		return poke_user(m, arg, (void *)&cinfo, sizeof(cinfo)) ? -EFAULT : 0;
	...
	case SNDCTL_DSP_GETTRIGGER:
		...
		return poke_user(m, arg, (void *)&dmabuf->trigger, sizeof(int));
		...
		for (i = 0; i < NR_HW_CH && card && !card->initializing; i++) {
			if (card->states[i] == NULL) {
				state = card->states[i] = (struct i810_state *)
					galloc(sizeof(struct i810_state));
				if (state == NULL)
					return -ENOMEM;
...
static void __init i810_configure_clocking (void)
{
	...
	if(card != NULL) {
		...
		//printk_djwill("offset = %d\n", offset);
		nexus_udelay(50000);
		new_offset = i810_get_dma_addr(state, 0);
		//printk_djwill("new offset = %d\n", new_offset);
		stop_dac(state);
		restore_intr(flags);
		i = new_offset - offset;
...
static int __init i810_probe(struct pci_dev *pci_dev, const struct pci_device_id *pci_id)
{
	...
	if ((card = galloc(sizeof(struct i810_card))) == NULL) {
	...
	//djwill: add two log regions for the IO ports
	if (0) {
	  kernel_log_io_add(pci_resource_start (pci_dev, 0), pci_resource_len(pci_dev, 0));
	  kernel_log_io_add(pci_resource_start (pci_dev, 1), pci_resource_len(pci_dev, 1));
	}
	...
	if (card->use_mmio) {
		if (request_mem_region(card->ac97base_mmio_phys, 512, "ich_audio MMBAR")) {
			//if ((card->ac97base_mmio = ioremap_protect(card->ac97base_mmio_phys, 512))) { /*@FIXME can ioremap fail? don't know (jsaw) */
			if ((card->ac97base_mmio = ioremap(card->ac97base_mmio_phys, 512))) { /*@FIXME can ioremap fail? don't know (jsaw) */
				if (request_mem_region(card->iobase_mmio_phys, 256, "ich_audio MBBAR")) {
					if ((card->iobase_mmio = ioremap(card->iobase_mmio_phys, 256))) {
						//if ((card->iobase_mmio = ioremap_protect(card->iobase_mmio_phys, 256))) {
	...
	if ((card->dev_audio = register_sound_dsp(&i810_audio_fops, -1)) < 0) {
 	...
 	card->initializing = 0;
	mycard = card;
	//register_adev(mycard, mycard->irq);
	//register_ndev(mycard, mycard->irq, i810_interrupt, "/dev/dsp", (DevOps *)&i810_devops);
	nexus_register_device(DEVICE_AUDIO, "i810", mycard->irq, &i810_devops,
			      i810_interrupt_nexus, NULL,
			      DRIVER_KERNEL);
	return 0;
