...
//DAN: these config paramaters should be on from config but they aren't
...
int __init vesafb_init(void)
{
	...
	if (!video_vbase) {
		...
		nexuslog("vesafb: abort, cannot ioremap video memory 0x%x @ 0x%lx\n",
	...
	nexuslog("vesafb: framebuffer at 0x%lx, mapped to 0x%p, size %dk\n",
	...
	if (video_bpp > 8) {
		...
		//XXX this case should never happen for us, but I don't 
		// know where the default stuff is coming from
#if 0
		for(i = 0; i < 16; i++) {
			j = color_table[i];
